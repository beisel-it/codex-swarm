...
type RepeatableRunDefinitionUpdate = z.infer<
  typeof repeatableRunDefinitionUpdateSchema
>;
type RepeatableRunTriggerCreate = RepeatableRunTriggerCreateInput;
type RepeatableRunTriggerUpdate = z.infer<
  typeof repeatableRunTriggerUpdateSchema
>;
...
type WorkerDispatchSessionAttach = z.infer<
  typeof workerDispatchSessionAttachSchema
>;
...
type AccessBoundary = Pick<
  ActorIdentity,
  "workspaceId" | "workspaceName" | "teamId" | "teamName"
> & {
...
type WorkerDispatchAssignmentRecord =
  typeof workerDispatchAssignments.$inferSelect;
type RepeatableRunDefinitionRecord =
  typeof repeatableRunDefinitions.$inferSelect;
...
const activeAgentStatuses = new Set<Agent["status"]>([
  "provisioning",
  "idle",
  "busy",
  "paused",
...
function readWorkerAssignmentInvalidationReason(
  metadata: Record<string, unknown> | null | undefined,
) {
...
function isInvalidatedWorkerDispatchAssignment(
  ...
  return (
    ...
    readWorkerAssignmentInvalidationReason(assignment.metadata) !== null
  );
}
...
): asserts access is AccessBoundary & {
  workspaceId: string;
  teamId: string;
} {
...
function requireAccessBoundary(
  access: AccessBoundary | undefined,
): OwnershipBoundary {
...
function workerNodeSupportsAssignment(
  workerNode: Pick<
    WorkerNode,
    "capabilityLabels" | "status" | "drainState" | "id"
  >,
  assignment: Pick<
    WorkerDispatchAssignment,
    "stickyNodeId" | "requiredCapabilities"
  >,
) {
  ...
  return assignment.requiredCapabilities.every((capability) =>
    workerNode.capabilityLabels.includes(capability),
  );
}

function workerDispatchRank(
  nodeId: string,
  assignment: Pick<
    WorkerDispatchAssignment,
    "stickyNodeId" | "preferredNodeId" | "createdAt"
  >,
) {
...
function isWorkerNodeEligible(
  workerNode: Pick<WorkerNode, "status" | "drainState">,
) {
...
function createRetentionWindowSummary(
  ...
  const expired = dates.filter(
    (value): value is Date =>
      value instanceof Date && value.getTime() < cutoff.getTime(),
  ).length;
...
function normalizeValidationTemplates(
  templates: TaskCreate["validationTemplates"] | undefined,
) {
...
function isApprovedHandoffApproval(
...
function isBudgetPolicyExceptionApproval(
  approval: Pick<Approval, "kind" | "requestedPayload" | "runId">,
  runId: string,
) {
  ...
  const parsedPayload = policyExceptionRequestPayloadSchema.safeParse(
    approval.requestedPayload,
  );

  if (parsedPayload.success) {
    return (
      parsedPayload.data.policyDecision.policyKey === "run_budget" &&
  ...
  return (
    ...
    (approval.requestedPayload as Record<string, unknown>).reason ===
      "budget_cap_exceeded"
  );
}
...
function readRunBudgetUsage(metadata: Record<string, unknown>) {
  ...
  return {
    tokensUsedTotal:
      typeof (budgetUsage as { tokensUsedTotal?: unknown }).tokensUsedTotal ===
      "number"
        ? Math.max(
            0,
            Math.trunc(
              (budgetUsage as { tokensUsedTotal: number }).tokensUsedTotal,
            ),
          )
        : 0,
    costUsdTotal:
      typeof (budgetUsage as { costUsdTotal?: unknown }).costUsdTotal ===
      "number"
...
function normalizeStoredRunContext(
  metadata: Record<string, unknown> | null | undefined,
): Run["context"] {
  ...
  const kind: Run["context"]["kind"] =
    rawKind === "project" ||
    (!rawKind && (projectId || projectSlug || projectName))
      ? "project"
      : "ad_hoc";
...
function normalizeStoredRunHandoffConfig(
  ...
  const parsed = runHandoffConfigSchema.safeParse(
    handoffConfig ?? defaultRunHandoffConfig,
  );
...
function normalizeStoredRunHandoffExecution(
  ...
  const parsed = runHandoffExecutionSchema.safeParse(
...
function summarizeValidationResults(
  validationsToSummarize: ValidationHistoryEntry[],
) {
  if (validationsToSummarize.length === 0) {
    return "No validations recorded";
  }

  const passed = validationsToSummarize.filter(
    (validation) => validation.status === "passed",
  ).length;
  const failed = validationsToSummarize.filter(
...
export class ControlPlaneService {
  ...
  async listProjects(access?: AccessBoundary): Promise<ProjectSummary[]> {
    const boundary = requireAccessBoundary(access);
    const [projectRows, repositoryRows, runRows, projectTeamRows] =
      await Promise.all([
        this.db
          .select()
          .from(projects)
    ...
    return projectRows.map((project) =>
      this.mapProjectSummary(project, repositoryRows, runRows, projectTeamRows),
    );
  }
    ...
    const projectTeamMemberRows =
      projectTeamRows.length === 0
        ...
        : await this.db
            ...
            .where(
              inArray(
                projectTeamMembers.projectTeamId,
                projectTeamRows.map((team) => team.id),
              ),
            )
            .orderBy(
              asc(projectTeamMembers.position),
              asc(projectTeamMembers.createdAt),
            );

    return this.mapProjectDetail(
      project,
      repositoryRows,
      runRows,
      projectTeamRows,
      projectTeamMemberRows,
    );
  }
    ...
    await this.db.transaction(async (tx) => {
      ...
      if (teamIds.length > 0) {
        const definitionRows = await tx
          .select({ id: repeatableRunDefinitions.id })
        ...
        if (definitionIds.length > 0) {
          await tx
            .delete(repeatableRunTriggers)
            .where(
              inArray(repeatableRunTriggers.repeatableRunId, definitionIds),
            );
          await tx
            .delete(repeatableRunDefinitions)
            .where(inArray(repeatableRunDefinitions.id, definitionIds));
        }
        await tx
          .delete(projectTeamMembers)
          .where(inArray(projectTeamMembers.projectTeamId, teamIds));
  ...
  async importProjectTeam(
    input: ProjectTeamImport,
    access?: AccessBoundary,
  ): Promise<ProjectTeamDetail> {
    const blueprintId = input.blueprintId ?? input.templateId;
    const blueprint = agentTeamBlueprints.find(
      (candidate) => candidate.id === blueprintId,
    );
    if (!blueprintId || !blueprint) {
      throw new HttpError(
        404,
        `team blueprint ${blueprintId ?? "unknown"} not found`,
      );
    }
    ...
    await this.db.transaction(async (tx) => {
      ...
      await this.replaceProjectTeamMembers(
        tx,
        teamId,
  ...
  async updateProjectTeam(
    ...
    if (input.name && updatedTeam) {
      ...
      await this.db
        .update(repeatableRunDefinitions)
        .set({
          projectTeamName: input.name,
          updatedAt: now,
        })
  ...
  async deleteProjectTeam(projectTeamId: string, access?: AccessBoundary) {
    ...
    const linkedDefinition = await this.db
      .select({
        id: repeatableRunDefinitions.id,
        name: repeatableRunDefinitions.name,
      })
      .from(repeatableRunDefinitions)
      .where(eq(repeatableRunDefinitions.projectTeamId, projectTeamId));
    if (linkedDefinition[0]) {
      throw new HttpError(
        409,
        `project team is still referenced by repeatable run ${linkedDefinition[0].name}`,
      );
    }
    const linkedRun = await this.db
      .select({ id: runs.id, goal: runs.goal })
      .from(runs)
      .where(eq(runs.projectTeamId, projectTeamId));
    if (linkedRun[0]) {
      throw new HttpError(
        409,
        `project team is still referenced by run ${linkedRun[0].goal}`,
      );
    }
  ...
  async registerWorkerNode(input: WorkerNodeRegister) {
    const now = this.clock.now();
    const [workerNode] = await this.db
      .insert(workerNodes)
      .values({
  ...
  async updateWorkerNodeDrainState(
    nodeId: string,
    input: WorkerNodeDrainUpdate,
  ) {
    await this.assertWorkerNodeExists(nodeId);
    const now = this.clock.now();
    const [workerNode] = await this.db
  ...
  async createRepository(input: RepositoryCreate, access?: AccessBoundary) {
    ...
    const now = this.clock.now();
    const approvalProfile = resolveRepositoryApprovalProfile(
      input,
      team.policyProfile,
    );
    ...
    const defaultBranch =
      input.defaultBranch ?? providerInspection.defaultBranch ?? "main";
  ...
  async updateRepository(
    ...
    const existingRepository = await this.assertRepositoryExists(
      repositoryId,
      access,
    );
    if (input.projectId) {
    ...
    const provider = (input.provider ??
      existingRepository.provider) as Repository["provider"];
    const url = input.url ?? existingRepository.url;
    const localPath =
      input.localPath === undefined
        ? existingRepository.localPath
        : input.localPath;
    const providerInspection = await inspectRepositoryProvider({
      provider,
      url,
      ...
      .set({
        ...
        projectId:
          input.projectId === undefined
            ? existingRepository.projectId
            : input.projectId,
        trustLevel: input.trustLevel ?? existingRepository.trustLevel,
        approvalProfile:
          input.approvalProfile ?? existingRepository.approvalProfile,
        providerSync: {
          connectivityStatus: providerInspection.connectivityStatus,
          validatedAt: providerInspection.validatedAt?.toISOString() ?? null,
          defaultBranch: providerInspection.defaultBranch ?? defaultBranch,
  ...
  async deleteRepository(repositoryId: string, access?: AccessBoundary) {
    ...
    const relatedRuns = await this.db
      .select({ id: runs.id })
      .from(runs)
    ...
    if (relatedRuns.length > 0) {
      throw new HttpError(
        409,
        "repository cannot be deleted while runs still reference it",
      );
    }
  ...
  async listRepeatableRunDefinitions(
    repositoryId?: string,
    access?: AccessBoundary,
  ) {
    const boundary = requireAccessBoundary(access);
    ...
    return rows.map((definition) =>
      this.mapRepeatableRunDefinition(definition),
    );
  }

  async createRepeatableRunDefinition(
    input: RepeatableRunDefinitionCreate,
    access?: AccessBoundary,
  ) {
    const boundary = requireAccessBoundary(access);
    const repository = await this.assertRepositoryExists(
      input.repositoryId,
      boundary,
    );
    const projectTeam = await this.assertProjectTeamExists(
      input.projectTeamId,
      boundary,
    );
    if (!repository.projectId) {
      throw new HttpError(
        409,
        "repeatable runs require a repository assigned to a project",
      );
    }
    if (projectTeam.projectId !== repository.projectId) {
      throw new HttpError(
        409,
        `project team ${projectTeam.id} does not belong to repository project ${repository.projectId}`,
      );
    }
    const now = this.clock.now();

    const [definition] = await this.db
      .insert(repeatableRunDefinitions)
      .values({
        id: crypto.randomUUID(),
    ...
    return this.mapRepeatableRunDefinition(
      expectPersistedRecord(definition, "repeatable run definition"),
    );
  }

  async updateRepeatableRunDefinition(
    repeatableRunId: string,
    input: RepeatableRunDefinitionUpdate,
    access?: AccessBoundary,
  ) {
    const existing = await this.assertRepeatableRunDefinitionExists(
      repeatableRunId,
      access,
    );
    ...
    const projectTeam = input.projectTeamId
      ...
      : existing.projectTeamId
        ...
        : null;
    const preservingLegacyTeamlessDefinition =
      !existing.projectTeamId &&
    ...
    const [definition] = await this.db
      ...
      .set({
        ...
        projectTeamName:
          projectTeam?.name ?? existing.projectTeamName ?? undefined,
        workspaceId: repository.workspaceId,
        teamId: repository.teamId,
        name: input.name ?? existing.name,
        description:
      ...
      .where(eq(repeatableRunDefinitions.id, repeatableRunId))
      .returning();
  ...
  async deleteRepeatableRunDefinition(
    repeatableRunId: string,
    access?: AccessBoundary,
  ) {
    ...
    if (relatedTriggers.length > 0) {
      throw new HttpError(
        409,
        "repeatable run cannot be deleted while triggers still reference it",
      );
    }
  ...
  async listRepeatableRunTriggers(
    ...
    const boundary = requireAccessBoundary(access);
    const definitions = await this.listRepeatableRunDefinitions(
      query.repositoryId,
      boundary,
    );
    const definitionIds = new Set(
      definitions
        .filter(
          (definition) =>
            !query.repeatableRunId || definition.id === query.repeatableRunId,
        )
  ...
  async createRepeatableRunTrigger(
    input: RepeatableRunTriggerCreate,
    access?: AccessBoundary,
  ) {
    const definition = await this.assertRepeatableRunDefinitionExists(
      input.repeatableRunId,
      access,
    );
    ...
    return this.mapRepeatableRunTrigger(
      expectPersistedRecord(trigger, "repeatable run trigger"),
    );
  }

  async updateRepeatableRunTrigger(
    triggerId: string,
    input: RepeatableRunTriggerUpdate,
    access?: AccessBoundary,
  ) {
    const existing = await this.assertRepeatableRunTriggerExists(
      triggerId,
      access,
    );
    const definition = input.repeatableRunId
      ? await this.assertRepeatableRunDefinitionExists(
          input.repeatableRunId,
          access,
        )
      : await this.assertRepeatableRunDefinitionExists(
          existing.repeatableRunId,
          access,
        );
    ...
    const [trigger] = await this.db
      ...
      .where(eq(repeatableRunTriggers.id, triggerId))
      .returning();
  ...
  async deleteRepeatableRunTrigger(triggerId: string, access?: AccessBoundary) {
    ...
    if (relatedReceipts.length > 0) {
      throw new HttpError(
        409,
        "repeatable run trigger cannot be deleted after receiving events",
      );
    }
  ...
  async listExternalEventReceipts(
    ...
    if (query.repositoryId) {
      ...
      conditions.push(
        eq(externalEventReceipts.repositoryId, query.repositoryId),
      );
    }

    if (query.repeatableRunId) {
      await this.assertRepeatableRunDefinitionExists(
        query.repeatableRunId,
        boundary,
      );
      conditions.push(
        eq(externalEventReceipts.repeatableRunId, query.repeatableRunId),
      );
    }

    if (query.repeatableRunTriggerId) {
      await this.assertRepeatableRunTriggerExists(
        query.repeatableRunTriggerId,
        boundary,
      );
      conditions.push(
        eq(
          externalEventReceipts.repeatableRunTriggerId,
          query.repeatableRunTriggerId,
        ),
      );
    }
  ...
  async listRuns(repositoryId?: string, access?: AccessBoundary) {
    ...
    if (repositoryId) {
      ...
      const rows = await this.db
        ...
        .orderBy(asc(runs.createdAt));
      await Promise.all(
        rows.map((run) => this.repairRunStateFromDispatchAssignments(run.id)),
      );
    ...
    const repositoriesById = new Map(
      (await this.listRepositories(boundary)).map(
    ...
    return refreshedRows.map((run) =>
      this.mapRun(
        run,
        repositoriesById.get(run.repositoryId)?.projectId ?? null,
      ),
    );
  }

  async listRunsByJobScope(
    repositoryId?: string,
    access?: AccessBoundary,
  ): Promise<RunsByJobScope> {
  ...
  async getRun(runId: string, access?: AccessBoundary): Promise<RunDetail> {
    ...
    const mappedSessions = runSessions.map(
      ({ session }): Session => this.mapSession(session),
    );

    return {
      ...this.mapRun(run, repository.projectId ?? null),
      tasks: mappedTasks,
      agents: this.mapAgents(
        runAgents,
        runSessions.map(({ session }) => session),
      ),
  ...
  async createRun(
    ...
    const projectId =
      input.projectId === undefined ? repository.projectId : input.projectId;
    if (projectId) {
      await this.assertProjectExists(projectId, access);
    }
    const projectTeam = input.projectTeamId
    ...
    if (projectTeam && projectTeam.projectId !== projectId) {
      throw new HttpError(
        409,
        `project team ${projectTeam.id} does not belong to project ${projectId}`,
      );
    }
  ...
  async ingestWebhook(input: IngestWebhookInput): Promise<IngestWebhookResult> {
    ...
    const repeatableRun = await this.assertRepeatableRunDefinitionExists(
      trigger.repeatableRunId,
    );
    const repository = await this.assertRepositoryExists(
      repeatableRun.repositoryId,
      {
    ...
    const deliveryId =
      headerValue(trigger.config.deliveryIdHeader) ?? crypto.randomUUID();
    ...
    const event = inboundWebhookEventEnvelopeSchema.parse({
      ...
      request: {
        ...
        deliveryId,
        signature:
          trigger.config.signatureHeader && signatureValue
            ? {
    ...
    const receiptId = expectPersistedRecord(
      receiptRow,
      "external event receipt",
    ).id;
    const rejectionReason = this.validateWebhookTriggerRequest(
      trigger,
      repeatableRun,
      input,
      event,
    ...
    if (rejectionReason) {
      const [updatedReceipt] = await this.db
      ...
      return {
        receipt: this.mapExternalEventReceipt(
          expectPersistedRecord(updatedReceipt, "external event receipt"),
        ),
        run: null,
      };
    }
      ...
      const run = await this.createRun(
        {
          ...
          planArtifactPath:
            repeatableRun.execution.planArtifactPath ?? undefined,
  ...
  async updateRunStatus(
    ...
    const [run] = await this.db
      ...
      .set({
        status: input.status,
        planArtifactPath:
          input.planArtifactPath === undefined
    ...
    const repository = await this.assertRepositoryExists(
      existingRun.repositoryId,
      access,
    );
    const mappedRun = this.mapRun(
      expectPersistedRecord(run, "run"),
      repository.projectId ?? null,
    );
  ...
  async updateRun(runId: string, input: RunUpdate, access?: AccessBoundary) {
    ...
    const nextProjectId =
      input.projectId === undefined ? existingRun.projectId : input.projectId;
    const nextProjectTeam =
      input.projectTeamId === undefined
        ? existingRun.projectTeamId
          ? await this.assertProjectTeamExists(
              existingRun.projectTeamId,
              access,
            )
          : null
    ...
    if (nextProjectTeam && nextProjectTeam.projectId !== nextProjectId) {
      throw new HttpError(
        409,
        `project team ${nextProjectTeam.id} does not belong to project ${nextProjectId}`,
      );
    }
    const now = this.clock.now();
    ...
    const handoff = resolveRunHandoffConfig(
      input.handoff,
      existingRun.handoffConfig,
    );
      ...
      .set({
        ...
        branchName:
          input.branchName === undefined
            ? existingRun.branchName
            : input.branchName,
        budgetTokens:
          input.budgetTokens === undefined
            ? existingRun.budgetTokens
            : input.budgetTokens,
        budgetCostUsd:
        ...
        policyProfile:
          input.policyProfile === undefined
            ? existingRun.policyProfile
            : input.policyProfile,
        handoffConfig: handoff,
        context:
          input.context === undefined ? existingRun.context : input.context,
        metadata:
  ...
  async recordRunBudgetCheckpoint(
    runId: string,
    input: RunBudgetCheckpoint,
    access?: AccessBoundary,
  ): Promise<RunBudgetState> {
    const run = await this.assertRunExists(runId, access);
    const now = this.clock.now();
    const currentUsage = readRunBudgetUsage(run.metadata);
    const tokensUsedTotal =
      currentUsage.tokensUsedTotal + input.tokensUsedDelta;
    ...
    if (
      budgetCostUsd !== null &&
      dollarsToStoredCents(costUsdTotal) >= run.budgetCostUsd!
    ) {
      exceeded.push("cost");
    }

    const policyExceptionRows = await this.db
      .select()
      .from(approvals)
      .where(
        and(eq(approvals.runId, runId), eq(approvals.kind, "policy_exception")),
      )
      .orderBy(asc(approvals.createdAt));
    const approvedBudgetException = policyExceptionRows.find(
      (approval) =>
        approval.status === "approved" &&
        isBudgetPolicyExceptionApproval(mapApprovalRecord(approval), runId),
    );
    const pendingBudgetException = policyExceptionRows.find(
      (approval) =>
    ...
    if (exceeded.length > 0) {
      if (approvedBudgetException) {
        ...
        const approval =
          pendingBudgetException ??
          expectPersistedRecord(
            (
              await this.db
                .insert(approvals)
                .values({
  ...
  async publishRunBranch(
    runId: string,
    input: RunBranchPublish,
    access?: AccessBoundary,
  ) {
    const existingRun = await this.assertRunExists(runId, access);
    const repository = await this.assertRepositoryExists(
    ...
    const branchApproval = await this.resolveRequiredHandoffApproval(
      runId,
      "patch",
      input.approvalId,
      access,
    ...
    const [run] = await this.db
      ...
      .set({
        ...
        branchPublishApprovalId:
          branchApproval?.id ?? existingRun.branchPublishApprovalId ?? null,
  ...
  async createRunPullRequestHandoff(
    runId: string,
    input: RunPullRequestHandoff,
    access?: AccessBoundary,
  ) {
    const now = this.clock.now();
    const run = await this.assertRunExists(runId, access);
    ...
    const mergeApproval = await this.resolveRequiredHandoffApproval(
      runId,
      "merge",
      input.approvalId,
      access,
    ...
    const updatedRun = await this.db.transaction(async (tx) => {
      const [persistedRun] = await tx
        ...
        .set({
          ...
          pullRequestApprovalId:
            mergeApproval?.id ?? run.pullRequestApprovalId ?? null,
          handoffStatus,
          updatedAt: now,
        })
        .where(eq(runs.id, runId))
    ...
    const taskRows = await this.db
      .select({
        ...
        latestVerificationChangeRequests:
          tasks.latestVerificationChangeRequests,
  ...
  async createTask(input: TaskCreate, access?: AccessBoundary) {
    ...
    const initialStatus = dependenciesReady
      ? "pending"
      : resolveInitialTaskStatus(input.dependencyIds);
    ...
    const [task] = await this.db
      ...
      .values({
        ...
        verificationStatus:
          definitionOfDone.length > 0 ? "pending" : "not_required",
        ...
        definitionOfDone,
        acceptanceCriteria,
        validationTemplates: normalizeValidationTemplates(
          input.validationTemplates,
        ),
        createdAt: now,
        updatedAt: now,
      })
  ...
  async updateTaskStatus(
    ...
    const ready = await this.areDependenciesSatisfied(
      task.runId,
      nextDependencyIds,
    );
    ...
    const effectiveStatus =
      ready && input.status === "blocked" ? "pending" : input.status;
    const now = this.clock.now();
  ...
  async createAgent(input: AgentCreate, access?: AccessBoundary) {
    ...
    const [agent] = await this.db.transaction(async (tx) => {
      ...
      if (input.session && createdAgent) {
        ...
        if (input.session.workerNodeId) {
          const workerNode = await this.assertWorkerNodeExists(
            input.session.workerNodeId,
          );
          const missingLabels = input.session.placementConstraintLabels.filter(
            (label) => !workerNode.capabilityLabels.includes(label),
          );
    ...
    const persistedSessions = await this.db
      .select()
      .from(sessions)
      .where(eq(sessions.agentId, persistedAgent.id))
      .orderBy(asc(sessions.createdAt));
  ...
  async createApproval(input: ApprovalCreate, access?: AccessBoundary) {
    ...
    if (input.kind === "policy_exception") {
      const parsedPolicyException =
        policyExceptionRequestPayloadSchema.safeParse(input.requestedPayload);
      ...
      if (parsedPolicyException.data.policyDecision.targetId !== input.runId) {
        throw new HttpError(
          409,
          "policy_exception approval targetId must match the approval runId",
        );
      }
    }
  ...
  async resolveApproval(
    ...
    if (existingApproval.kind === "policy_exception") {
      const parsedResolution =
        policyExceptionResolutionPayloadSchema.safeParse(resolutionPayload);

      if (!parsedResolution.success) {
        throw new HttpError(
          400,
          "policy_exception approvals require an explicit resolution outcome",
        );
      }
      ...
      if (parsedResolution.data.outcome !== expectedOutcome) {
        throw new HttpError(
          409,
          `policy_exception resolution outcome must be ${expectedOutcome} when status is ${input.status}`,
        );
      }
    }
  ...
  async createValidation(input: ValidationCreate, access?: AccessBoundary) {
    ...
    const template = input.templateName
      ? (task?.validationTemplates.find(
          (candidate) => candidate.name === input.templateName,
        ) ?? null)
      : null;
    ...
    if (input.templateName && !task) {
      throw new HttpError(
        400,
        "taskId is required when templateName is provided",
      );
    }
    ...
    if (input.templateName && !template) {
      throw new HttpError(
        404,
        `validation template ${input.templateName} not found`,
      );
    }
    ...
    return this.hydrateValidationHistoryEntry(
      expectPersistedRecord(validation, "validation"),
    );
  }

  async listValidations(
    query: ValidationListQuery | string,
    access?: AccessBoundary,
  ): Promise<ValidationHistoryEntry[]> {
    const { runId, taskId } =
      typeof query === "string" ? { runId: query, taskId: undefined } : query;
  ...
  async createWorkerDispatchAssignment(input: WorkerDispatchCreate) {
    ...
    const [assignment] = await this.db
      .insert(workerDispatchAssignments)
      .values({
        id: crypto.randomUUID(),
        runId: input.runId,
        taskId: input.taskId,
    ...
    return this.mapWorkerDispatchAssignment(
      expectPersistedRecord(assignment, "worker dispatch assignment"),
    );
  }
  ...
  async attachSessionToWorkerDispatchAssignment(
    ...
    const assignment = this.mapWorkerDispatchAssignment(
      await this.assertWorkerDispatchAssignmentExists(assignmentId),
    );
    ...
    if (session.agentId !== assignment.agentId) {
      throw new HttpError(
        409,
        `session ${sessionId} does not belong to agent ${assignment.agentId}`,
      );
    }

    const [updatedAssignment] = await this.db
      .update(workerDispatchAssignments)
      .set({
        sessionId,
        updatedAt: this.clock.now(),
      })
      .where(eq(workerDispatchAssignments.id, assignmentId))
      .returning();

    return this.mapWorkerDispatchAssignment(
      expectPersistedRecord(updatedAssignment, "worker dispatch assignment"),
    );
  }

  private async loadRunProjectTeam(
  ...
  private selectProjectTeamMemberForRole(
    ...
    const assignmentCounts = new Map<string, number>(
      candidates.map((member) => [member.id, 0]),
    );
    for (const agent of runAgents) {
      if (
        !agent.projectTeamMemberId ||
        !assignmentCounts.has(agent.projectTeamMemberId)
      ) {
        continue;
      }
      assignmentCounts.set(
        agent.projectTeamMemberId,
        (assignmentCounts.get(agent.projectTeamMemberId) ?? 0) + 1,
      );
    }
  ...
  private selectVerifierProjectTeamMember(
    ...
    const workerAgent =
      runAgents.find((agent) => agent.id === workerAgentId) ?? null;
    const workerMemberId = workerAgent?.projectTeamMemberId ?? null;
    const explicitReviewer = selectCandidate(
      preferredReviewerRoles,
      workerMemberId,
    );
    ...
    const reviewLikeRoles = projectTeam.members
      ...
      .filter(
        (role) =>
          !preferredReviewerRoles.includes(role) &&
          normalizeReviewLikeRole(role),
      );
  ...
  async enqueueRunnableWorkerDispatches(
    runId: string,
    access?: AccessBoundary,
  ) {
    const runDetail = await this.getRun(runId, access);
    const repository = this.mapRepository(
      await this.assertRepositoryExists(runDetail.repositoryId, access),
    );
    const projectTeam = await this.loadRunProjectTeam(
      runDetail.projectTeamId,
      access,
    );
    ...
    const availableSlots = Math.max(
      0,
      runDetail.concurrencyCap - activeExecutionTaskIds.size,
    );
    const tasksToQueue = runDetail.tasks
      .filter(
        (task) =>
          task.status === "pending" && !activeAssignmentTaskIds.has(task.id),
      )
      .sort((left, right) => {
    ...
    const leaderAgent =
      runDetail.agents.find((agent) => agent.role === "tech-lead") ?? null;
    const queuedAssignments: WorkerDispatchAssignment[] = [];
    const workerSandbox =
      process.env.CODEX_SWARM_WORKER_SANDBOX?.trim() || "workspace-write";
    const workerApprovalPolicy =
      process.env.CODEX_SWARM_WORKER_APPROVAL_POLICY?.trim() || "on-request";
    const workspaceProvisioningMode = resolveWorkspaceProvisioningMode();

    for (const task of tasksToQueue) {
      const projectTeamMember = this.selectProjectTeamMemberForRole(
        projectTeam,
        task.role,
        runDetail.agents,
      );
      if (runDetail.projectTeamId && !projectTeamMember) {
        throw new HttpError(
          409,
          `project team ${runDetail.projectTeamName ?? runDetail.projectTeamId} has no member for role ${task.role}`,
        );
      }
        ...
        (await this.createAgent(
          {
            ...
            name: (
              projectTeamMember?.name ?? `${task.role}-${task.title}`
            ).slice(0, 72),
  ...
  async claimNextWorkerDispatch(nodeId: string) {
    const workerNode = this.mapWorkerNode(
      await this.assertWorkerNodeExists(nodeId),
    );
    const now = this.clock.now();
    ...
    const candidates = rows
      ...
      .filter((assignment) => {
        ...
        const preferredWorkerNode = availableWorkerNodes
          .filter((candidate) =>
            workerNodeSupportsAssignment(candidate, assignment),
          )
          .sort((left, right) =>
            compareWorkerNodesForAssignment(left, right, assignment, now),
          )[0];
    ...
    for (const candidate of candidates) {
      if (normalizeAssignmentKind(candidate.metadata) !== "verification") {
        ...
        const resolution = task
          ...
          : {
              valid: false,
              effectiveTaskStatus: null as Task["status"] | null,
            };
    ...
    if (!nextAssignment) {
      await Promise.all(
        [...invalidatedRunIds].map((runId) =>
          this.reconcileRunExecutionState(runId),
        ),
      );
      return null;
    }
    ...
    const [updatedAssignment] = await this.db
      ...
      .where(eq(workerDispatchAssignments.id, nextAssignment.id))
      .returning();
  ...
  async completeWorkerDispatch(
    ...
    if (
      assignment.claimedByNodeId &&
      assignment.claimedByNodeId !== input.nodeId
    ) {
      throw new HttpError(
        409,
        `worker dispatch assignment ${assignmentId} is claimed by a different node`,
      );
    }
  ...
  async reconcileWorkerNode(
    nodeId: string,
    input: WorkerNodeReconcile,
  ): Promise<WorkerNodeReconcileReport> {
    await this.assertWorkerNodeExists(nodeId);
    const now = this.clock.now();
    ...
    const claimedAssignments = await this.db
      .select()
      .from(workerDispatchAssignments)
      .where(
        and(
      ...
      .orderBy(asc(workerDispatchAssignments.createdAt));
    const nodeAssignments = claimedAssignments.map((assignment) =>
      this.mapWorkerDispatchAssignment(assignment),
    );
    ...
    const strandedSessions = await this.db
      .select()
      .from(sessions)
    ...
    const orphanSessions = strandedSessions.filter(
      (session) => !trackedSessionIds.has(session.id),
    );
  ...
  async getTuiOverview(access?: AccessBoundary): Promise<TuiOverview> {
    ...
    const scopedDispatchAssignments = dispatchAssignments.filter((assignment) =>
    ...
    for (const approval of approvalsList) {
      approvalsByRunId.set(approval.runId, [
        ...(approvalsByRunId.get(approval.runId) ?? []),
        approval,
      ]);
    }

    for (const assignment of scopedDispatchAssignments) {
      dispatchByRunId.set(assignment.runId, [
        ...(dispatchByRunId.get(assignment.runId) ?? []),
        assignment,
      ]);
    }
    ...
    const runSummaries = runsList.map((run, index): TuiOverviewRun => {
      ...
      if (!repository) {
        throw new HttpError(
          500,
          `repository ${run.repositoryId} missing from TUI overview aggregation`,
        );
      }
      ...
      for (const assignment of runDispatchAssignments) {
        if (assignment.state === "queued") {
          ...
          assignment.state === "failed" &&
          !isInvalidatedWorkerDispatchAssignment(assignment)
        ) {
      ...
      return {
        ...
        validationCounts,
        dispatchCounts,
        activeSessionCount: runDetail.sessions.filter(
          (session) => session.state === "active",
        ).length,
        workerNodeIds,
        blockedTaskIds: runDetail.tasks
          .filter((task) => task.status === "blocked")
          .map((task) => task.id),
        pendingApprovalIds: runApprovals
          .filter((approval) => approval.status === "pending")
          .map((approval) => approval.id),
        failedValidationIds: validations
    ...
    for (const runSummary of runSummaries) {
      if (
        runSummary.run.status === "awaiting_approval" ||
        runSummary.approvalCounts.pending > 0
      ) {
        alerts.push({
      ...
      for (const task of runDetails.find(
        (detail) => detail.id === runSummary.run.id,
      )?.tasks ?? []) {
      ...
      for (const validation of validationsByRun[
        runsList.findIndex((run) => run.id === runSummary.run.id)
      ] ?? []) {
    ...
    return {
      ...
      summary: {
        ...
        runsActive: runsList.filter((run) => isActiveRunStatus(run.status))
          .length,
        approvalsPending: approvalsList.filter(
          (approval) => approval.status === "pending",
        ).length,
        validationsFailed: validationsByRun
          .flat()
          .filter((validation) => validation.status === "failed").length,
        tasksBlocked: runDetails
          .flatMap((runDetail) => runDetail.tasks)
          .filter((task) => task.status === "blocked").length,
        workerNodesOnline: workerNodesList.filter(
          (workerNode) => workerNode.status === "online",
        ).length,
        workerNodesDegraded: workerNodesList.filter(
          (workerNode) => workerNode.status === "degraded",
        ).length,
        workerNodesOffline: workerNodesList.filter(
          (workerNode) => workerNode.status === "offline",
        ).length,
        dispatchQueued: scopedDispatchAssignments.filter(
          (assignment) => assignment.state === "queued",
        ).length,
        dispatchRetrying: scopedDispatchAssignments.filter(
          (assignment) => assignment.state === "retrying",
        ).length,
      },
      runs: runSummaries,
      fleet: {
  ...
  async getTuiRunDrilldown(
    runId: string,
    access?: AccessBoundary,
  ): Promise<TuiRunDrilldown> {
    const [
      runDetail,
      approvalsList,
      validationsList,
    ...
    const repositoryRecord = await this.assertRepositoryExists(
      runDetail.repositoryId,
      access,
    );
    ...
    return {
      ...
      workerNodes: allWorkerNodes.filter((workerNode) =>
        workerNodeIds.includes(workerNode.id),
      ),
      dispatchAssignments,
      events: eventRows.map((event) =>
        controlPlaneEventSchema.parse(normalizeLegacyEventActor(event)),
      ),
    };
  }
  ...
  async exportRunAudit(
    ...
    return {
      ...
      retention: {
        ...
        runs: createRetentionWindowSummary(
          [runDetail.completedAt ?? runDetail.createdAt],
          retentionPolicy.runsDays,
          now,
        ),
        artifacts: createRetentionWindowSummary(
          artifactsList.map((artifact) => artifact.createdAt),
          retentionPolicy.artifactsDays,
          now,
        ),
        events: createRetentionWindowSummary(
          mappedEvents.map((event) => event.createdAt),
          retentionPolicy.eventsDays,
          now,
        ),
      },
      exportedAt: now,
  ...
  async getGovernanceAdminReport(input: {
    ...
    const [repositoryRows, runRows, approvalRows] = await Promise.all([
      this.db
        ...
        .where(
          and(
            eq(repositories.workspaceId, input.access.workspaceId),
            eq(repositories.teamId, input.access.teamId),
          ),
        )
        .orderBy(asc(repositories.createdAt)),
      input.runId
    ...
    const [artifactRows, eventRows] =
      governedRunIds.length === 0
        ? [[], []]
        : await Promise.all([
            this.db
              .select()
    ...
    const repositoriesById = new Map(
      repositoryRows.map(
        (repository) =>
          [repository.id, this.mapRepository(repository)] as const,
      ),
    );
    ...
    const approvalHistory = approvalRows
      ...
      .map((approval) => {
        ...
        const repository = run
          ? repositoriesById.get(run.repositoryId)
          : undefined;
    ...
    const repositoryProfiles = [...repositoriesById.values()].reduce<
      Map<
        string,
        { profile: string; repositoryCount: number; runCount: number }
      >
    >((acc, repository) => {
      ...
      entry.runCount += runRows.filter(
        (run) => run.repositoryId === repository.id,
      ).length;
    ...
    return {
      ...
      retention: {
        ...
        runs: createRetentionWindowSummary(
          runRows.map((run) => run.completedAt ?? run.createdAt),
          input.retentionPolicy.runsDays,
          now,
        ),
        artifacts: createRetentionWindowSummary(
          artifactRows.map((artifact) => artifact.createdAt),
          input.retentionPolicy.artifactsDays,
          now,
        ),
        events: createRetentionWindowSummary(
          eventRows.map((event) => event.createdAt),
          input.retentionPolicy.eventsDays,
          now,
        ),
      },
      approvals: {
        total: approvalRows.length,
        pending: approvalRows.filter(
          (approval) => approval.status === "pending",
        ).length,
        approved: approvalRows.filter(
          (approval) => approval.status === "approved",
        ).length,
        rejected: approvalRows.filter(
      ...
      policies: {
        ...
        sensitiveRepositories: [...repositoriesById.values()]
          .filter(
            (repository) =>
              repository.trustLevel !== "trusted" ||
              repository.approvalProfile !== "standard",
          )
          .map((repository) => ({
            repositoryId: repository.id,
            repositoryName: repository.name,
      ...
      for (const run of runsRows) {
        const expiresAt = new Date(
          (run.completedAt ?? run.createdAt).getTime() +
            input.retentionPolicy.runsDays * 24 * 60 * 60 * 1000,
        );
        await this.db
          .update(runs)
          .set({
      ...
      for (const artifact of artifactRows) {
        const expiresAt = new Date(
          artifact.createdAt.getTime() +
            input.retentionPolicy.artifactsDays * 24 * 60 * 60 * 1000,
        );
        await this.db
          .update(artifacts)
          .set({
      ...
      for (const event of eventRows) {
        const expiresAt = new Date(
          event.createdAt.getTime() +
            input.retentionPolicy.eventsDays * 24 * 60 * 60 * 1000,
        );
        await this.db
          .update(controlPlaneEvents)
          .set({
  ...
  async getRepositorySecretAccessPlan(input: {
    ...
    const repository = this.mapRepository(
      await this.assertRepositoryExists(input.repositoryId, input.access),
    );
    const trustAllowed = input.secrets.allowedRepositoryTrustLevels.includes(
      repository.trustLevel,
    );
    const sensitivePolicy = input.secrets.sensitivePolicyProfiles.includes(
  ...
  async runCleanupJob(input: CleanupJobRunInput): Promise<CleanupJobReport> {
    ...
    const rowBySessionId = new Map(
      rows.map((row) => [row.sessionId, row] as const),
    );
    const worktreeCleanup =
      input.deleteStaleWorktrees &&
      resolveWorkspaceProvisioningMode() === "isolated"
        ? await cleanupWorktreePaths(
            recoveryPlan
              .filter(
                (item) =>
                  item.action === "mark_stale" || item.action === "archive",
              )
              .map((item) => {
    ...
    const cleanupByPath = new Map(
      worktreeCleanup.map((item) => [item.path, item] as const),
    );
    ...
    return {
      ...
      markedStale: recoveryPlan.filter((item) => item.action === "mark_stale")
        .length,
      ...
      worktreeDeleteFailures: worktreeCleanup.filter(
        (item) => !item.deleted && item.reason !== "placeholder_path",
      ).length,
      items: recoveryPlan.map((item) => {
        const row = expectPersistedRecord(
          rowBySessionId.get(item.sessionId),
          "cleanup session row",
        );
  ...
  private async resolveValidationArtifactIds(
    ...
    const clauses = [
      ...
      eq(artifacts.path, artifactPath),
    ];
  ...
  private async ensureOwnershipBoundary(
    access: OwnershipBoundary,
  ): Promise<TeamRecord> {
    const now = this.clock.now();
  ...
  private assertBoundaryMatch(
    ...
    if (access.workspaceId !== workspaceId || access.teamId !== teamId) {
      throw new HttpError(
        403,
        `${entityType} ${entityId} is outside the caller boundary`,
      );
    }
  }
  ...
  private async assertProjectTeamExists(
    projectTeamId: string,
    access?: AccessBoundary,
  ) {
    const [projectTeam] = await this.db
      .select()
      .from(projectTeams)
      .where(eq(projectTeams.id, projectTeamId));
  ...
  private async assertArtifactsBelongToRun(
    ...
    if (rows.length !== artifactIds.length) {
      throw new HttpError(
        404,
        "one or more validation artifacts were not found",
      );
    }
    ...
    if (rows.some((artifact) => artifact.runId !== runId)) {
      throw new HttpError(
        409,
        "validation artifacts must belong to the same run",
      );
    }
  }
  ...
  private async resolveRequiredHandoffApproval(
    ...
    const approvalsOfKind = approvalsForRun.filter(
      (approval) => approval.kind === kind,
    );

    if (approvalId) {
      const approval = approvalsOfKind.find(
      ...
      if (!isApprovedHandoffApproval(approval, kind, runId)) {
        throw new HttpError(
          409,
          `${kind} approval ${approvalId} must be approved before handoff`,
        );
      }
    ...
    throw new HttpError(
      409,
      `${kind} approval linkage is required for this handoff`,
    );
  }

  private async hydrateValidationHistory(
    rows: (typeof validations.$inferSelect)[],
  ): Promise<ValidationHistoryEntry[]> {
    if (rows.length === 0) {
      return [];
    }
  ...
  private async hydrateValidationHistoryEntry(
    row: typeof validations.$inferSelect,
  ): Promise<ValidationHistoryEntry> {
  ...
  private mapProjectSummary(
    ...
    return {
      ...
      teamCount: projectTeamRows.filter((team) => team.projectId === project.id)
        .length,
      repositoryCount: repositoryRows.filter(
  ...
  private mapProjectRepositoryAssignment(
    projectId: string,
    repository: typeof repositories.$inferSelect,
  ): ProjectRepositoryAssignment {
    return {
      projectId,
      repositoryId: repository.id,
      repository: this.mapRepository(repository),
  ...
  private mapProjectRunAssignment(
    projectId: string,
    run: typeof runs.$inferSelect,
  ): ProjectRunAssignment {
    return {
      projectId,
      runId: run.id,
      run: {
  ...
  private mapProjectDetail(
    ...
    return {
      ...
      repositoryAssignments: repositoryRows
        ...
        .map((repository) =>
          this.mapProjectRepositoryAssignment(project.id, repository),
        ),
  ...
  private mapProjectTeamMember(
    record: ProjectTeamMemberRecord,
  ): ProjectTeamMember {
    return {
      ...record,
  ...
  private mapProjectTeamDetail(
    ...
    return projectTeamDetailSchema.parse({
      ...
      memberCount: memberRows.filter(
        (member) => member.projectTeamId === record.id,
      ).length,
    });
  }
  ...
  private mapRepository(
    ...
    return {
      ...
      providerSync: {
        ...
        validatedAt: providerSync.validatedAt
          ? new Date(providerSync.validatedAt)
          : null,
  ...
  private mapTask(task: typeof tasks.$inferSelect): Task {
    return {
      ...
      latestVerificationChangeRequests:
        task.latestVerificationChangeRequests ?? [],
  ...
  private buildApprovalAuditEntries(
    ...
    return approvalsList.map((approval) =>
      this.buildApprovalAuditEntry(approval, run, repository, events),
    );
  }
  ...
  private buildApprovalAuditEntry(
    ...
    return {
      ...
      resolvedByDelegate:
        approval.delegation?.delegateActorId === approval.resolver,
  ...
  private buildVerifierAssignmentPrompt(
    run: RunDetail,
    repository: Repository,
    task: Task,
  ...
  private async enqueueVerifierAssignment(input: {
    ...
    const now = this.clock.now();
    const workerAgent =
      input.run.agents.find(
        (agent) => agent.id === input.workerAssignment.agentId,
      ) ?? null;
    ...
    const fallbackReviewerAgent =
      input.run.agents.find(
        (agent) =>
          agent.id !== input.workerAssignment.agentId &&
          (preferredReviewerRoles.includes(agent.role) ||
            normalizeReviewLikeRole(agent.role)),
      ) ?? null;
    const fallbackRole =
      verifierMember?.role ?? fallbackReviewerAgent?.role ?? input.task.role;
    const fallbackProfile =
      verifierMember?.profile ??
    ...
    const verifierAgent = await this.createAgent(
      {
        ...
        name: (
          verifierMember?.name ?? `${fallbackRole}-${input.task.title}-verifier`
        ).slice(0, 72),
        role: fallbackRole,
    ...
    await this.recordControlPlaneEvent(
      controlPlaneEventDefinitions.taskVerificationRequested,
      {
        runId: input.run.id,
  ...
  private async repairRunStateFromDispatchAssignments(runId: string) {
    ...
    const sessionIdsToStop = new Map<
      string,
      { workerNodeId: string | null; stickyNodeId: string | null }
    >();
    const sessionIdsToActivate = new Map<
      string,
      { workerNodeId: string | null; stickyNodeId: string | null }
    >();
    ...
    for (const task of taskRows) {
      ...
      if (assignment.state === "completed") {
        if (assignmentKind === "verification") {
          const verificationStatus =
            task.verificationStatus as TaskVerificationStatus;
          ...
          if (
            ...
            task.ownerAgentId !== assignment.agentId ||
            task.verifierAgentId !== assignment.agentId
          ) {
            await this.db
              .update(tasks)
              .set({
      ...
      if (assignment.state === "claimed") {
        if (assignmentKind === "verification") {
          ...
          const resolution = await this.resolveWorkerAssignmentTaskState(
            task,
            assignment.state,
          );

          if (!resolution.valid) {
            await this.invalidateWorkerDispatchAssignment(
              this.mapWorkerDispatchAssignment(assignment),
              {
      ...
      if (assignment.state === "queued" || assignment.state === "retrying") {
        if (assignmentKind === "verification") {
          if (
            task.status !== "awaiting_review" ||
            task.ownerAgentId !== assignment.agentId ||
            task.verificationStatus !== "requested" ||
            task.verifierAgentId !== assignment.agentId
          ) {
            await this.db
              .update(tasks)
      ...
      if (assignment.state === "failed") {
        if (
          assignmentKind !== "verification" &&
          isInvalidatedWorkerDispatchAssignment(
            this.mapWorkerDispatchAssignment(assignment),
          )
        ) {
          agentIdsToIdle.add(assignment.agentId);
    ...
    const activeAssignments = refreshedAssignments.filter(
      (assignment) =>
        assignment.state === "queued" ||
        assignment.state === "claimed" ||
        assignment.state === "retrying",
    );
    const anyFailedTask = refreshedTasks.some(
      (task) => task.status === "failed",
    );
    const allTasksCompleted =
      refreshedTasks.length > 0 &&
      refreshedTasks.every((task) => task.status === "completed");
    const currentStatus = run.status as Run["status"];
    const nextStatus: Run["status"] = anyFailedTask
      ? "failed"
      : allTasksCompleted
        ? "completed"
        : refreshedTasks.length > 0 &&
  ...
  async reconcileRunExecutionState(runId: string, access?: AccessBoundary) {
    ...
    const anyFailedTask = runDetail.tasks.some(
      (task) => task.status === "failed",
    );
    const allTasksCompleted =
      runDetail.tasks.length > 0 &&
      runDetail.tasks.every((task) => task.status === "completed");
    const nextStatus: Run["status"] = anyFailedTask
      ? "failed"
      : allTasksCompleted
        ? "completed"
  ...
  private async recordControlPlaneEvent(
    definition: {
      ...
      entityType: ControlPlaneEvent["entityType"];
    },
    input: {
      runId?: string | null;
      entityId: string;
  ...
  private async resolveRunWorkspacePath(
    runId: string,
  ...
  private async pickApprovedHandoffApprovalId(
    ...
    const approved = approvalsForRun
      .filter(
        (approval) => approval.kind === kind && approval.status === "approved",
      )
      .sort(
  ...
  private buildAutoHandoffPullRequestContent(
    ...
    return this.listRunValidations(runDetail.id).then((validations) => {
      ...
      const title =
        renderHandoffTemplate(handoff.titleTemplate, templateValues) ??
        runDetail.goal;
      const body =
        renderHandoffTemplate(handoff.bodyTemplate, templateValues) ??
        [
          `## Summary`,
          runDetail.goal,
          ``,
  ...
  async maybeExecuteAutoHandoff(runId: string, access?: AccessBoundary) {
    ...
    if (
      ...
      handoffExecution.state === "in_progress" ||
      handoffExecution.state === "failed"
    ) {
      return runDetail;
    }
    ...
    try {
      const workspacePath = await this.resolveRunWorkspacePath(
        runId,
        repository,
      );
      const { baseBranch, branchName, title, body } =
        await this.buildAutoHandoffPullRequestContent(
          runDetail,
          runTasks,
          repository,
          handoff,
        );
      const patchApprovalId = await this.pickApprovedHandoffApprovalId(
        runId,
        "patch",
        access,
      );
      const mergeApprovalId = await this.pickApprovedHandoffApprovalId(
        runId,
        "merge",
        access,
      );
      ...
      if (handoff.autoCreatePullRequest && !runDetail.pullRequestUrl) {
        ...
        await this.createRunPullRequestHandoff(
          runId,
          {
            title,
            body,
  ...
  private mapAgents(agentRows: AgentRecord[], sessionRows: SessionRecord[]) {
    ...
    return agentRows.map((agent) =>
      this.mapAgent(agent, sessionsByAgentId.get(agent.id) ?? []),
    );
  }
  ...
  private buildAgentObservability(
    agent: AgentRecord,
    agentSessions: SessionRecord[],
  ): Agent["observability"] {
    const orderedSessions = [...agentSessions].sort(
    ...
    for (let index = orderedSessions.length - 1; index >= 0; index -= 1) {
      ...
      visibleTranscriptUpdatedAt =
        transcript[transcript.length - 1]?.createdAt ?? null;
      break;
    }

    const fallbackVisibleSession =
    ...
    if (currentSession) {
      return {
        ...
        currentSessionState:
          currentSession.state as Agent["observability"]["currentSessionState"],
        visibleTranscriptSessionId: visibleTranscriptSession?.id ?? null,
        visibleTranscriptSessionState: (visibleTranscriptSession?.state ??
          null) as Agent["observability"]["visibleTranscriptSessionState"],
        visibleTranscriptUpdatedAt,
        lineageSource:
          visibleTranscriptSession &&
          visibleTranscriptSession.id !== currentSession.id
            ? "session_rollover"
            : "active_session",
      };
    }
    ...
    if (fallbackVisibleSession) {
      return {
        ...
        visibleTranscriptSessionState:
          fallbackVisibleSession.state as Agent["observability"]["visibleTranscriptSessionState"],
        visibleTranscriptUpdatedAt,
  ...
  private async transitionWorkerDispatchFailureOrCompletion(
    ...
    const assignmentKind =
      input.outcome?.kind ?? normalizeAssignmentKind(assignment.metadata);
    ...
    if (input.status === "completed") {
      ...
      if (
        ...
        if (verificationStatus === "passed") {
          await this.maybeUnblockDependentTasks(
            assignment.runId,
            assignment.taskId,
            "completed",
          );
        ...
        if (workerOutcome.outcomeStatus === "completed") {
          if (taskRequiresVerification(this.mapTask(task))) {
            ...
            const repository = this.mapRepository(
              await this.assertRepositoryExists(runDetail.repositoryId),
            );
            const projectTeam = await this.loadRunProjectTeam(
              runDetail.projectTeamId,
            );
    ...
    await this.db
      ...
      .set({
        ...
        latestVerificationSummary:
          assignmentKind === "verification" && !canRetry
  ...
  private buildTaskDag(runTasks: Task[]): TaskDagGraph {
    ...
    const edgeId = (sourceTaskId: string, targetTaskId: string) =>
    ...
    for (const task of runTasks) {
      blockedByTaskIds.set(
        task.id,
        task.dependencyIds.filter((dependencyId) => {
    ...
    const edges: TaskDagGraph["edges"] = runTasks.flatMap((task) =>
        ...
        return [
          {
            ...
            isBlocking: (blockedByTaskIds.get(task.id) ?? []).includes(
              dependencyId,
            ),
          },
        ];
    ...
    const rootTaskIds = nodes
      .filter((node) => node.isRoot)
      .map((node) => node.taskId);
    const blockedTaskIds = nodes
    ...
    const unblockPaths: TaskDagGraph["unblockPaths"] = blockedTaskIds.map(
      (taskId) => {
        ...
        return {
          ...
          pathTaskIds: runTasks
            .map((task) => task.id)
            .filter((candidateId) => pathTaskIds.has(candidateId)),
          pathEdgeIds: edges
            .map((edge) => edge.id)
            .filter((candidateId) => pathEdgeIds.has(candidateId)),
        };
      },
    );
  ...
  private mapWorkerNode(
    workerNode: typeof workerNodes.$inferSelect,
  ): WorkerNode {
    return {
      ...workerNode,
  ...
  private async resolveWebhookTriggerByPath(
    endpointPath: string,
    ...
    if (matches.length === 0) {
      throw new HttpError(
        404,
        `no webhook trigger is configured for ${endpointPath}`,
      );
    }
    ...
    if (matches.length > 1) {
      throw new HttpError(
        409,
        `multiple webhook triggers are configured for ${endpointPath}`,
      );
    }
  ...
  private async assertRepeatableRunDefinitionExists(
    repeatableRunId: string,
    access?: AccessBoundary,
    ...
    if (!definition) {
      throw new HttpError(
        404,
        `repeatable run ${repeatableRunId} was not found`,
      );
    }
    ...
    if (access) {
      ...
      if (
        definition.workspaceId !== boundary.workspaceId ||
        definition.teamId !== boundary.teamId
      ) {
        throw new HttpError(
          404,
          `repeatable run ${repeatableRunId} was not found`,
  ...
  private async assertRepeatableRunTriggerExists(
    triggerId: string,
    access?: AccessBoundary,
  ): Promise<RepeatableRunTrigger> {
    const rows = await this.db
    ...
    if (!trigger) {
      throw new HttpError(
        404,
        `repeatable run trigger ${triggerId} was not found`,
      );
    }
    ...
    if (access) {
      ...
      if (
        trigger.workspaceId !== boundary.workspaceId ||
        trigger.teamId !== boundary.teamId
      ) {
        throw new HttpError(
          404,
          `repeatable run trigger ${triggerId} was not found`,
  ...
  private validateWebhookTriggerRequest(
    ...
    if (
      !trigger.config.allowedMethods.includes(input.method as "POST" | "PUT")
    ) {
    ...
    if (
      ...
      !trigger.config.filters.eventNames.includes(event.eventName ?? "")
    ) {
    ...
    if (
      ...
      !trigger.config.filters.actions.includes(event.action ?? "")
    ) {
    ...
    if (
      ...
      !trigger.config.filters.branches.includes(branch ?? "")
    ) {
    ...
    if (Object.keys(trigger.config.filters.metadata).length > 0) {
      ...
      for (const [key, value] of Object.entries(
        trigger.config.filters.metadata,
      )) {
  ...
  private mapRepeatableRunDefinition(
    record: RepeatableRunDefinitionRecord,
  ...
  private mapRepeatableRunTrigger(
    record: RepeatableRunTriggerRecord,
  ): RepeatableRunTrigger {
    return repeatableRunTriggerSchema.parse(record);
  }

  private mapExternalEventReceipt(
    record: typeof externalEventReceipts.$inferSelect,
  ...
  private resolveRunJobScope(
    ...
    if (runProjectId) {
      ...
      return {
        ...
        repositoryProjectId,
        reason:
          repositoryProjectId === runProjectId
            ? "run_assigned"
  ...
  private async assertDependenciesBelongToRun(
    runId: string,
    ...
    const foreignDependency = dependencyTasks.find(
      (dependencyTask) => dependencyTask.runId !== runId,
    );
  ...
  private async invalidateWorkerDispatchAssignment(
    ...
    await this.recordControlPlaneEvent(
      controlPlaneEventDefinitions.workerDispatchAssignmentUpdated,
      {
        runId: assignment.runId,
        entityId: assignment.id,
        status: "failed",
  ...
  private async areDependenciesSatisfied(
    ...
    const dependencyTasks = await this.db
      ...
      .from(tasks)
      .where(and(eq(tasks.runId, runId), inArray(tasks.id, dependencyIds)));

    return (
      dependencyTasks.length === dependencyIds.length &&
      dependencyTasks.every((dependency) => dependency.status === "completed")
    );
  }

  private async maybeUnblockDependentTasks(
    runId: string,
    ...
    for (const candidateTask of candidateTasks) {
      if (
        candidateTask.status !== "blocked" ||
        !candidateTask.dependencyIds.includes(completedTaskId)
      ) {
        continue;
      }
