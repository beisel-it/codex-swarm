...
function resolveAssignmentExecutionProfile(
...
function toSessionRecord(session: Session, worktreePath: string) {
  return {
    ...
    lastHeartbeatAt: toDate(
      (session as unknown as Record<string, unknown>).lastHeartbeatAt as
        | Date
        | string
        | null
        | undefined,
    ...
    createdAt:
      toDate(
        (session as unknown as Record<string, unknown>).createdAt as
          | Date
          | string
          | null
          | undefined,
    ...
    updatedAt:
      toDate(
        (session as unknown as Record<string, unknown>).updatedAt as
          | Date
          | string
          | null
          | undefined,
...
function resolveAssignmentKind(assignment: WorkerDispatchAssignment) {
  return assignment.metadata?.assignmentKind === "verification"
    ? "verification"
    : "worker";
}
...
function buildInboundMessages(
  ...
  const agentNames = new Map(
    runDetail.agents.map((agent) => [agent.id, agent.name] as const),
  );
  ...
  return messages
    ...
    .map((message) => ({
      sender: message.senderAgentId
        ? (agentNames.get(message.senderAgentId) ?? message.senderAgentId)
        : "system",
      body: message.body,
    }));
}
...
function buildVerifierMessages(
  ...
  const workerAgentId =
    typeof assignment.metadata?.workerAgentId === "string"
...
async function publishWorkerOutcomeMessages(
  ...
  const leaderAgent =
    runDetail.agents.find(
      (agent) => agent.role === "tech-lead" && agent.id !== assignment.agentId,
    ) ?? null;
  ...
  for (const message of outcome.messages) {
    ...
    if (message.target.startsWith("role:")) {
      ...
      const matchingAgents = runDetail.agents.filter(
        (agent) => agent.role === role && agent.id !== assignment.agentId,
      );
...
async function runLeaderResliceLoopSafely(
  ...
  try {
    ...
    const leaderAgent =
      runDetail.agents.find(
        (agent) =>
          agent.role === "tech-lead" && agent.id !== assignment.agentId,
      ) ?? null;
    ...
    await postRunMessage(request, {
      ...
      senderAgentId: assignment.agentId,
      ...(leaderAgent
        ? { recipientAgentId: leaderAgent.id, kind: "direct" as const }
...
function toLeaderResliceOutcomeFromVerification(
  outcome: VerifierTaskOutcome,
): WorkerTaskOutcome | null {
  if (outcome.status === "passed") {
    return null;
  }

  if (outcome.status === "blocked") {
    const hasActionableBlocker =
      outcome.blockingIssues.length > 0 || outcome.messages.length > 0;
  ...
  const hasReworkSignal =
    outcome.changeRequests.length > 0 || outcome.findings.length > 0;
  ...
  return {
    ...
    messages: outcome.messages,
    blockingIssues: [...outcome.changeRequests, ...outcome.findings],
    ...(outcome.artifacts ? { artifacts: outcome.artifacts } : {}),
  };
}
...
async function synchronizeRunBranchContext(
  ...
  await request("PATCH", `/api/v1/runs/${runDetail.id}`, {
    branchName,
  });
...
async function recordWorkerOutcomeArtifacts(
  ...
  for (const artifact of outcome.artifacts ?? []) {
    ...
    const contentBase64 =
      ...
      (await readFile(resolvedArtifactPath, "base64").catch(
        (error: unknown) => {
          if (
            error &&
            typeof error === "object" &&
            ...
            throw new Error(
              `worker outcome artifact source file not found: ${resolvedArtifactPath}`,
            );
          }
...
async function recordWorkerOutcomeHandoff(
  ...
  const effectiveBranch =
    outcome.branchPublish?.branchName ??
  ...
  if (outcome.branchPublish) {
    await request("POST", `/api/v1/runs/${assignment.runId}/publish-branch`, {
      branchName: effectiveBranch,
      publishedBy: assignment.agentId,
      ...(outcome.branchPublish.commitSha
        ? { commitSha: outcome.branchPublish.commitSha }
        : {}),
      ...(outcome.branchPublish.notes
        ? { notes: outcome.branchPublish.notes }
        : {}),
    });
  }
  ...
  if (outcome.pullRequestHandoff) {
    await request(
      ...
      `/api/v1/runs/${assignment.runId}/pull-request-handoff`,
      {
        ...
        ...(outcome.pullRequestHandoff.baseBranch
          ? { baseBranch: outcome.pullRequestHandoff.baseBranch }
          : {}),
        headBranch: outcome.pullRequestHandoff.headBranch ?? effectiveBranch,
        ...(outcome.pullRequestHandoff.url
          ? { url: outcome.pullRequestHandoff.url }
          : {}),
        ...(outcome.pullRequestHandoff.number
          ? { number: outcome.pullRequestHandoff.number }
          : {}),
        ...(outcome.pullRequestHandoff.status
          ? { status: outcome.pullRequestHandoff.status }
          : {}),
      },
    );
  }
...
function buildVerifierArtifactContexts(taskArtifacts: Artifact[]) {
  return taskArtifacts.map((artifact) => ({
    ...
    contentType: artifact.contentType,
    summary:
      typeof artifact.metadata?.source === "string"
        ? `source=${artifact.metadata.source}`
        : null,
  }));
}

function buildVerifierValidationContexts(
  taskValidations: ValidationHistoryEntry[],
) {
...
export async function runManagedWorkerDispatch(
  ...
  const repositories = await input.request<Repository[]>(
    "GET",
    "/api/v1/repositories",
  );
  ...
  if (!repository) {
    const updated = await failAssignment(
      input.request,
      assignment,
      input.nodeId,
  ...
  const effectiveBranchName =
    runDetail.branchName ?? assignment.branchName ?? repository.defaultBranch;
  ...
  try {
    const runMessages = await input.request<RunMessage[]>(
      "GET",
      `/api/v1/messages?runId=${assignment.runId}`,
    );
    ...
    const effectivePrompt = task
      ? assignmentKind === "verification"
        ? buildVerifierTaskExecutionPrompt({
            ...
            taskRole: task.role,
            taskDescription: [task.description, assignment.prompt]
              .filter(Boolean)
              .join("\n\nOperator brief:\n"),
            definitionOfDone: task.definitionOfDone,
            acceptanceCriteria: task.acceptanceCriteria,
            workerSummary:
              typeof assignment.metadata?.workerSummary === "string"
                ? assignment.metadata.workerSummary
                : "No worker summary was captured.",
            artifacts: buildVerifierArtifactContexts(
              (
                await input.request<Artifact[]>(
                  "GET",
                  `/api/v1/artifacts?runId=${assignment.runId}`,
                )
              ).filter((artifact) => artifact.taskId === task.id),
            ),
            validations: buildVerifierValidationContexts(
              (
                await input.request<ValidationHistoryEntry[]>(
                  "GET",
                  `/api/v1/validations?runId=${assignment.runId}`,
                )
              ).filter((validation) => validation.taskId === task.id),
            ),
            relevantMessages: buildVerifierMessages(
              runDetail,
              assignment,
              runMessages,
            ),
    ...
    let supervisorStatus: WorkerDispatchOrchestrationResult["supervisorStatus"] =
      "stopped";
    ...
    if (persistedSession?.threadId) {
      ...
      supervisorStatus =
        stopped.supervisor.status === "failed" ? "failed" : "stopped";
    } else {
    ...
    if (responseOutput && assignment.taskId) {
      if (assignmentKind === "verification") {
        ...
        const synchronizedBranchName = await synchronizeRunBranchContext(
          input.request,
          runDetail,
          workspace.path,
        );
        ...
        if (
          outcome.status === "needs_slicing" ||
          (outcome.status === "blocked" && outcome.blockerKind === "actionable")
        ) {
          await runLeaderResliceLoopSafely(
            input.request,
            runDetail,
