...
export const repositoryRoutes: FastifyPluginAsync = async (app) => {
  app.get("/repositories", async (_request, reply) => {
    try {
      ...
      if (
        ...
        app.observability.recordRecoverableDatabaseFallback(
          "repositories.list",
          error,
        );
    ...
    return app.observability.withTrace(
      ...
      async () => {
        ...
        await app.observability.recordTimelineEvent(
          timelineEvent(controlPlaneEvents.repositoryCreated, {
  ...
  app.patch("/repositories/:id", async (request) => {
    return app.observability.withTrace(
      "api.repositories.update",
      async () => {
        const { id } = idParamSchema.parse(request.params);
        const input = repositoryUpdateSchema.parse(request.body);
        return app.controlPlane.updateRepository(
          id,
          input,
          request.authContext,
        );
