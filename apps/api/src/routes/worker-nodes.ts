...
export const workerNodeRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/worker-nodes", async (request, reply) => {
    return app.observability.withTrace("api.worker-nodes.register", async () => {
  ...
  app.patch("/worker-nodes/:id/heartbeat", async (request) => {
    return app.observability.withTrace("api.worker-nodes.heartbeat", async () => {
  ...
  app.patch("/worker-nodes/:id/drain", async (request) => {
    return app.observability.withTrace("api.worker-nodes.drain", async () => {
      const { id } = idParamSchema.parse(request.params);
