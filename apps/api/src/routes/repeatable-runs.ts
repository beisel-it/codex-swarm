...
export const repeatableRunRoutes: FastifyPluginAsync = async (app) => {
  app.get("/repeatable-runs", async (request) => {
    ...
    return app.controlPlane.listRepeatableRunDefinitions(query.repositoryId, request.authContext);
  });

  app.post("/repeatable-runs", async (request, reply) => {
    const input = repeatableRunDefinitionCreateSchema.parse(request.body);
    const definition = await app.controlPlane.createRepeatableRunDefinition(input, request.authContext);
  ...
  app.patch("/repeatable-runs/:id", async (request) => {
    ...
    return app.controlPlane.updateRepeatableRunDefinition(id, input, request.authContext);
  });

  app.delete("/repeatable-runs/:id", async (request, reply) => {
    const { id } = idParamSchema.parse(request.params);
    await app.controlPlane.deleteRepeatableRunDefinition(id, request.authContext);
    return reply.code(204).send();
  });

  app.get("/repeatable-run-triggers", async (request) => {
    const query = repeatableRunTriggerListQuerySchema.parse(request.query ?? {});
    return app.controlPlane.listRepeatableRunTriggers(query, request.authContext);
  });

  app.post("/repeatable-run-triggers", async (request, reply) => {
    const input = repeatableRunTriggerCreateSchema.parse(request.body);
    const trigger = await app.controlPlane.createRepeatableRunTrigger(input, request.authContext);
  ...
  app.patch("/repeatable-run-triggers/:id", async (request) => {
    ...
    return app.controlPlane.updateRepeatableRunTrigger(id, input, request.authContext);
  });
  ...
  app.get("/external-event-receipts", async (request) => {
    const query = externalEventReceiptListQuerySchema.parse(request.query ?? {});
    return app.controlPlane.listExternalEventReceipts(query, request.authContext);
  });
};
