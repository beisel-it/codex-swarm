...
function parseRoles(
  ...
  const primaryRole = (explicitRole ??
    headerRoles[0] ??
    fallbackRole) as GovernanceRole;
...
export const authPlugin = fp(async (app: FastifyInstance) => {
  app.addHook("onRequest", async (request: FastifyRequest) => {
    ...
    request.authContext = {
      principal:
        request.headers["x-codex-principal"]?.toString() ??
        app.config.DEV_AUTH_PRINCIPAL,
      actorId:
        request.headers["x-codex-actor-id"]?.toString() ??
        app.config.DEV_AUTH_ACTOR_ID,
      actorType:
        request.headers["x-codex-actor-type"]?.toString() === "service" ||
        request.headers["x-codex-actor-type"]?.toString() === "system"
          ? (request.headers["x-codex-actor-type"]?.toString() as
              | "service"
              | "system")
          : "user",
      email:
        request.headers["x-codex-email"]?.toString() ??
        app.config.DEV_AUTH_EMAIL ??
        null,
      role: primaryRole,
      roles,
      workspaceId:
        request.headers["x-codex-workspace-id"]?.toString() ??
        app.config.DEV_AUTH_WORKSPACE_ID,
      workspaceName:
        request.headers["x-codex-workspace-name"]?.toString() ??
        app.config.DEV_AUTH_WORKSPACE_NAME,
      teamId:
        request.headers["x-codex-team-id"]?.toString() ??
        app.config.DEV_AUTH_TEAM_ID,
      teamName:
        request.headers["x-codex-team-name"]?.toString() ??
        app.config.DEV_AUTH_TEAM_NAME,
      policyProfile:
        request.headers["x-codex-policy-profile"]?.toString() ??
        app.config.DEV_AUTH_POLICY_PROFILE,
    };
    app.observability.setActorContext(request.authContext);
  });
});
