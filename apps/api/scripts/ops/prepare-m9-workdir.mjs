...
const runLabel = process.env.M9_RUN_LABEL ?? new Date().toISOString().replace(/[:.]/g, "-");
...
const manifest = {
  ...
  scope: "M9 readiness only; this directory is prepared for the future scenario run and does not start it.",
  paths: {
    runRoot,
    workspaceRoot,
    artifactsRoot,
  ...
  requiredEnv: {
    ...
    M9_REPOSITORY_URL: "<set if the scenario uses a provider-backed sample repo>",
