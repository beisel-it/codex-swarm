...
function createSession(id: string, state: "pending" | "active" | "stopped" | "failed" | "stale" | "archived", overrides: Partial<Record<string, unknown>> = {}) {
  return {
    id,
...
describe("ControlPlaneService agent observability mapping", () => {
  it("preserves transcript visibility through task state transitions even without persisted transcript entries", () => {
    ...
    const stoppedSession = createSession("44444444-4444-4444-8444-444444444444", "stopped");

    const observability = (service as any).buildAgentObservability(agent, [stoppedSession]);
  ...
  it("keeps the latest retry session linked while retaining older visible transcript lineage", () => {
    ...
    const previousSession = createSession("55555555-5555-4555-8555-555555555555", "stale", {
      metadata: {
        transcript: [
          {
    ...
    const retrySession = createSession("66666666-6666-4666-8666-666666666666", "pending", {
      createdAt: new Date("2026-03-29T09:20:00.000Z"),
      updatedAt: new Date("2026-03-29T09:20:00.000Z")
    });

    const observability = (service as any).buildAgentObservability(agent, [previousSession, retrySession]);
    ...
    expect(observability.visibleTranscriptUpdatedAt).toEqual(new Date("2026-03-29T09:15:00.000Z"));
  });
  ...
  it("maps active-agent payloads with fallback session lineage for restart recovery responses", () => {
    ...
    const staleSession = createSession("88888888-8888-4888-8888-888888888888", "stale", {
      staleReason: "node_lost:heartbeat expired",
      createdAt: new Date("2026-03-29T09:05:00.000Z")
    });

    const mappedAgents = (service as any).mapAgents([restartingAgent], [staleSession]);
