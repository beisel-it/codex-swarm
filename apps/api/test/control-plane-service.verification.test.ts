    ...
    return {
      set: (values: Record<string, unknown>) => ({
        where: (condition: {
          ...
          if (table === runs) {
            const record = this.runStore.find(
              (candidate) => candidate.id === id,
            );
            Object.assign(record, values);
            return {
      ...
      from: (table: unknown) => ({
        where: (condition: {
          ...
          orderBy: async () => {
            ...
            if (table === runs) {
              return this.runStore.filter((candidate) =>
            ...
            if (table === tasks) {
              return this.taskStore.filter(
                (candidate) =>
                  ids.includes(candidate.id) || ids.includes(candidate.runId),
              );
            }
          ...
          then: <TResult1 = any[], TResult2 = never>(
            ...
            const rows =
              table === runs
                ? this.runStore.filter((candidate) =>
                    ids.includes(candidate.id),
                  )
                : table === tasks
    ...
    return {
      set: (values: Record<string, unknown>) => ({
        where: (condition: {
          ...
          const records = store.filter((candidate) =>
            ids.includes(candidate.id),
          );
...
describe("ControlPlaneService verification lifecycle", () => {
  it("routes worker completion into awaiting_review and queues a verifier assignment", async () => {
    ...
    const createWorkerDispatchAssignment = vi.fn(
    ...
    (service as any).createWorkerDispatchAssignment =
    ...
    await (service as any).transitionWorkerDispatchFailureOrCompletion(
      workerAssignment,
      {
        nodeId: "node-1",
        status: "completed",
    ...
    expect(taskRecord.latestVerificationSummary).toContain(
    ...
    expect(createWorkerDispatchAssignment).toHaveBeenCalledWith(
      expect.objectContaining({
        taskId: "task-1",
  ...
  it("marks verifier failures on the task without creating follow-up work directly", async () => {
    ...
    expect(taskRecord.latestVerificationSummary).toBe(
  ...
  it("marks verifier passes as completed and unblocks downstream work", async () => {
    ...
    expect(maybeUnblockDependentTasks).toHaveBeenCalledWith(
      "run-1",
      "task-1",
      "completed",
    );
  ...
  it("keeps runs in progress while work awaits review and avoids duplicate queueing for the review-gated task", async () => {
    ...
    (service as any).createAgent = vi.fn(
      async (input: Record<string, unknown>) => {
        const agent = {
          id: "worker-agent-2",
          runId: "run-1",
    ...
    expect(createWorkerDispatchAssignment).not.toHaveBeenCalledWith(
      expect.objectContaining({
        taskId: "task-review",
      }),
    );
