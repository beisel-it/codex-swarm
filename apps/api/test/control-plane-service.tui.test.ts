...
describe("ControlPlaneService TUI aggregates", () => {
  it("builds a single overview payload for runs, review state, and fleet health", async () => {
    ...
    const overview = await ControlPlaneService.prototype.getTuiOverview.call(
      service,
      access,
    );
  ...
  it("builds a run drilldown payload with review and fleet context", async () => {
    ...
    const drilldown =
      await ControlPlaneService.prototype.getTuiRunDrilldown.call(
        service,
        runId,
        access,
      );
...
describe("ControlPlaneService task DAG shaping", () => {
  it("derives nodes, dependency edges, roots, blocked tasks, and unblock paths", () => {
    ...
    expect(
      taskDag.nodes.find(
        (node: { taskId: string }) =>
          node.taskId === "22222222-2222-4222-8222-222222222222",
      ),
    ).toMatchObject({
    ...
    expect(
      taskDag.nodes.find(
        (node: { taskId: string }) =>
          node.taskId === "33333333-3333-4333-8333-333333333333",
      ),
    ).toMatchObject({
    ...
    expect(taskDag.unblockPaths).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
