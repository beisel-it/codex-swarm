...
describe("ControlPlaneService auto handoff", () => {
  it("publishes the branch and opens a GitHub PR when auto handoff is enabled", async () => {
    const db = new FakeAutoHandoffDb([
      createTaskRow(),
      createTaskRow({ id: crypto.randomUUID(), title: "Collect validation evidence" })
    ]);
    ...
    const publishRunBranch = vi.fn(async (_runId: string, input: Record<string, unknown>) => {
    ...
    const createRunPullRequestHandoff = vi.fn(async (_runId: string, input: Record<string, unknown>) => {
    ...
    (service as any).updateRunHandoffExecutionState = async (_runId: string, execution: Record<string, unknown>) => {
    ...
    (service as any).recordControlPlaneEvent = async (event: { eventType: string }) => {
      eventTypes.push(event.eventType);
    };
    (service as any).resolveRunWorkspacePath = async () => "/tmp/codex-swarm-auto-handoff";
    (service as any).pickApprovedHandoffApprovalId = async (_runId: string, kind: string) => (
      kind === "patch"
        ? "patch-approval"
        : "merge-approval"
    );
    ...
    expect(publishRunBranch).toHaveBeenCalledWith("run-1", expect.objectContaining({
      branchName: "runs/auto-handoff",
      publishedBy: "system:auto-handoff",
      approvalId: "patch-approval"
    }), undefined);
    expect(createGitHubPullRequest).toHaveBeenCalledWith(expect.objectContaining({
      workspacePath: "/tmp/codex-swarm-auto-handoff",
      baseBranch: "main",
    ...
    expect(result.pullRequestUrl).toBe("https://github.com/example/codex-swarm/pull/42");
  });
