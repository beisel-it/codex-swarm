...
export function deriveBoardModel(
  ...
  const runApprovals = data.approvals.filter(
    (approval) => approval.runId === selectedRun?.id,
  );
  const runValidations = data.validations.filter(
    (validation) => validation.runId === selectedRun?.id,
  );
  const runMessages = data.messages.filter(
    (message) => message.runId === selectedRun?.id,
  );
  const runSessions = data.sessions.filter((session) =>
    data.agents.some(
      (agent) =>
        agent.id === session.agentId && agent.runId === selectedRun?.id,
    ),
  );
  ...
  const pendingApprovals = runApprovals.filter(
    (approval) => approval.status === "pending",
  );
  const failedValidations = runValidations.filter(
    (validation) => validation.status === "failed",
  );
  const fleetIssues = runWorkerNodes.filter(
    (node) => node.status !== "online" || node.drainState !== "active",
  );
  const staleSessions = runSessions.filter(
    (session) => session.state === "stale" || session.state === "failed",
  );
  ...
  return {
    ...
    stats: [
      ...
      },
      {
        ...
        tone:
          fleetIssues.length + staleSessions.length > 0 ? "danger" : "success",
    ...
    alerts: [
      ...
      ...staleSessions.map((session) => ({
        ...
        detail:
          session.staleReason ??
          "Session placement requires operator attention.",
        tone: "danger" as const,
      })),
    ].slice(0, 8),
...
function describeFleetIssue(node: WorkerNodeRecord) {
  const reason =
    typeof node.metadata.drainReason === "string"
      ? node.metadata.drainReason
      : null;
