...
describe("single-host installer assets", () => {
  it("renders home-relative env defaults", () => {
    ...
    expect(rendered).toContain(
      "/tmp/codex-home/.local/share/codex-swarm/artifacts",
    );
  ...
  it("renders systemd templates with explicit install paths", () => {
    ...
    expect(rendered).toContain(
      "EnvironmentFile=/home/test/.config/codex-swarm/single-host.env",
    );
  });

  it("detects placeholder values before service start", () => {
    expect(
      envContainsInstallPlaceholders("CODEX_SWARM_DB_PASSWORD=change-me\n"),
    ).toBe(true);
    expect(
      envContainsInstallPlaceholders(
        "CODEX_SWARM_WORKSPACE_ROOT=__HOME__/workspaces\n",
      ),
    ).toBe(true);
    expect(
      envContainsInstallPlaceholders("CODEX_SWARM_DB_PASSWORD=ready\n"),
    ).toBe(false);
  });
