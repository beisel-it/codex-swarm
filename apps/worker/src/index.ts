...
export type { CodexServerConfig, CodexToolExecutor } from "./runtime.js";
...
if (process.argv[1] && import.meta.url.endsWith(process.argv[1])) {
  ...
  const postgresUrl =
    process.env.CODEX_SWARM_DATABASE_URL ??
    process.env.CODEX_SWARM_POSTGRES_URL;
  ...
  if (controlPlaneUrl && nodeId && workspaceRoot && redisUrl && postgresUrl) {
    const runtime = {
      ...
      codexCommand: process.env.CODEX_SWARM_CODEX_COMMAND
        ...
        : ["codex"],
      codexTransport:
        process.env.CODEX_SWARM_MCP_TRANSPORT === "streamable_http"
          ? {
              kind: "streamable_http" as const,
              url: process.env.CODEX_SWARM_MCP_SERVER_URL ?? "",
              headers: {},
              protocolVersion:
                process.env.CODEX_SWARM_MCP_PROTOCOL_VERSION ?? "2025-11-25",
            }
          : {
              kind: "stdio" as const,
            },
      ...
      heartbeatIntervalSeconds: Number(
        process.env.CODEX_SWARM_HEARTBEAT_INTERVAL_SECONDS ?? "30",
      ),
    };

    const authToken =
      process.env.CODEX_SWARM_AUTH_TOKEN ?? process.env.DEV_AUTH_TOKEN;
