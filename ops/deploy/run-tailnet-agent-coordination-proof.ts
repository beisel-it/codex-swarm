...
function parseCodexCommand(value: string | undefined) {
  ...
  if (trimmed.startsWith("[") && trimmed.endsWith("]")) {
    ...
    if (
      ...
      throw new Error(
        "CODEX_SWARM_CODEX_COMMAND must be a non-empty string array when using JSON form",
      );
    }
  ...
  const runtime = new CodexSessionRuntime({
    ...
    supervisor: new CodexServerSupervisor({
      config: {
        ...
        sandbox:
          process.env.CODEX_SWARM_LEADER_SANDBOX?.trim() ||
          "danger-full-access",
        approvalPolicy:
          process.env.CODEX_SWARM_LEADER_APPROVAL_POLICY?.trim() || "never",
...
async function main() {
  ...
  const authToken =
    process.env.CODEX_SWARM_API_TOKEN?.trim() ||
  ...
  const proofRoot = await mkdtemp(
    join(tmpdir(), "codex-swarm-agent-coordination-proof-"),
  );
  const repoRoot = join(proofRoot, "repo");
  const leaderWorkspace = process.cwd();
  await mkdir(repoRoot, { recursive: true });
  const handoffRepo = await ensureGithubRepository(
    process.env.CODEX_SWARM_HANDOFF_REPO?.trim() ||
      "beisel-it/codex-swarm-e2e-handoff",
  );
  const remoteUrl = handoffRepo.url.endsWith(".git")
    ? handoffRepo.url
    : `${handoffRepo.url}.git`;

  await writeFile(
    join(repoRoot, "README.md"),
    "# Hosted coordination proof\n",
    "utf8",
  );
  await writeFile(join(repoRoot, "docs", ".gitkeep"), "", "utf8").catch(
    async () => {
      await mkdir(join(repoRoot, "docs"), { recursive: true });
      await writeFile(join(repoRoot, "docs", ".gitkeep"), "", "utf8");
    },
  );
  await writeFile(join(repoRoot, "stories", ".gitkeep"), "", "utf8").catch(
    async () => {
      await mkdir(join(repoRoot, "stories"), { recursive: true });
      await writeFile(join(repoRoot, "stories", ".gitkeep"), "", "utf8");
    },
  );
  await writeFile(join(repoRoot, "notes", ".gitkeep"), "", "utf8").catch(
    async () => {
      await mkdir(join(repoRoot, "notes"), { recursive: true });
      await writeFile(join(repoRoot, "notes", ".gitkeep"), "", "utf8");
    },
  );
  await writeFile(join(repoRoot, "ops", ".gitkeep"), "", "utf8").catch(
    async () => {
  ...
  await runCommand("git", [
    ...
    "config",
    "user.email",
    "codex-swarm@example.com",
  ]);
  await runCommand("git", ["-C", repoRoot, "remote", "remove", "origin"]).catch(
    () => undefined,
  );
  await runCommand("git", [
    "-C",
  ...
  const leaderAgent = await api<Agent>(
    baseUrl,
    authToken,
    "POST",
    "/api/v1/agents",
  ...
  const oversizedTask = await api<Task>(
    baseUrl,
    authToken,
    "POST",
    "/api/v1/tasks",
  ...
  const writerTask = await api<Task>(
    baseUrl,
    authToken,
    "POST",
    "/api/v1/tasks",
      ...
      validationTemplates: [
        {
          name: "writer-note",
          command:
            "grep -Fqx 'Changelog references the approved Story Alpha hero line.' notes/changelog.txt",
  ...
  await api<Task>(baseUrl, authToken, "POST", "/api/v1/tasks", {
    ...
    description:
      "Create ops/deployment.txt containing exactly 'Tailnet-only API and frontend; data services stay local-only.'",
    ...
    validationTemplates: [
      {
        name: "ops-note",
        command:
          "grep -Fqx 'Tailnet-only API and frontend; data services stay local-only.' ops/deployment.txt",
  ...
  await api<Task>(baseUrl, authToken, "POST", "/api/v1/tasks", {
    ...
    description:
      "Create docs/api-summary.txt containing exactly 'API summary confirmed for hosted coordination proof.'",
    ...
    validationTemplates: [
      {
        name: "api-summary",
        command:
          "grep -Fqx 'API summary confirmed for hosted coordination proof.' docs/api-summary.txt",
  ...
  while (Date.now() < deadline) {
    const runDetail = await api<RunDetail>(
      baseUrl,
      authToken,
      "GET",
      `/api/v1/runs/${run.id}`,
    );
    const assignments = await api<WorkerDispatchAssignment[]>(
      baseUrl,
      authToken,
      "GET",
      `/api/v1/worker-dispatch-assignments?runId=${run.id}`,
    );
    const claimedNow = assignments.filter(
    ...
    if (runDetail.status === "completed") {
      ...
      const childTasks = detail.tasks.filter(
        (task) => task.parentTaskId === oversizedTask.id,
      );
      const writerAgent = detail.agents.find(
        (agent) => agent.currentTaskId === writerTask.id,
      );
      ...
      const writerTranscript = writerSession
        ? await api<Array<{ kind: string; text: string }>>(
            baseUrl,
            authToken,
            "GET",
            `/api/v1/sessions/${writerSession.id}/transcript`,
          )
        : [];
      const oversizedAgent = detail.agents.find(
        (agent) => agent.currentTaskId === oversizedTask.id,
      );
      const oversizedSession = oversizedAgent
        ? detail.sessions.find(
            (session) => session.agentId === oversizedAgent.id,
          )
        : null;
      const oversizedTranscript = oversizedSession
        ? await api<Array<{ kind: string; text: string }>>(
            ...
            authToken,
            "GET",
            `/api/v1/sessions/${oversizedSession.id}/transcript`,
          )
        : [];

      const hasLeaderToWorker = messages.some(
        (message) =>
          message.kind === "direct" && message.senderAgentId === leaderAgent.id,
      );
      const hasWorkerToLeader = messages.some(
        (message) =>
          message.kind === "direct" &&
          message.recipientAgentId === leaderAgent.id &&
          message.senderAgentId !== leaderAgent.id,
      );
      const hasWorkerToWriter = writerAgent
        ? messages.some(
            (message) =>
              message.kind === "direct" &&
              message.recipientAgentId === writerAgent.id &&
              message.senderAgentId !== leaderAgent.id,
          )
        : false;
      const writerSawPeerMessage = writerTranscript.some(
        (entry) =>
          entry.kind === "prompt" &&
          entry.text.includes(
            "Use the approved Story Alpha hero line in the changelog note.",
          ),
      );
      const oversizedRequestedSlicing =
        oversizedTranscript.some(
          (entry) =>
            entry.kind === "response" &&
            entry.text.includes('"status":"needs_slicing"'),
        ) ||
        oversizedTranscript.some(
          (entry) =>
            entry.kind === "response" &&
      ...
      if (!hasLeaderToWorker) {
        throw new Error(
          "Proof failed: no leader=>worker direct message was persisted",
        );
      }

      if (!hasWorkerToLeader) {
        throw new Error(
          "Proof failed: no worker=>leader direct message was persisted",
        );
      }

      if (!hasWorkerToWriter) {
        throw new Error(
          "Proof failed: no worker=>worker direct message was persisted",
        );
      }

      if (!writerSawPeerMessage) {
        throw new Error(
          "Proof failed: writer transcript did not contain inbound peer coordination",
        );
      }

      if (!oversizedRequestedSlicing) {
        throw new Error(
          "Proof failed: oversized worker transcript did not record needs_slicing output",
        );
      }

      if (childTasks.length < 2) {
        throw new Error(
          `Proof failed: expected at least 2 child tasks from leader reslice, saw ${childTasks.length}`,
        );
      }

      if (claimedNodeSnapshots.size < 2) {
        throw new Error(
          `Proof failed: expected work across multiple nodes, saw ${claimedNodeSnapshots.size}`,
        );
      }

      if (maxSimultaneousClaims < 2) {
        throw new Error(
          `Proof failed: expected at least 2 simultaneous claims, saw ${maxSimultaneousClaims}`,
        );
      }

      if (completedAssignments.length < 4) {
        throw new Error(
          `Proof failed: expected at least 4 completed assignments, saw ${completedAssignments.length}`,
        );
      }

      const alphaHeroAgent = detail.agents.find((agent) =>
        detail.tasks.some(
          (task) =>
            task.id === agent.currentTaskId &&
            task.title.toLowerCase().includes("hero"),
        ),
      );
      const alphaCtaAgent = detail.agents.find((agent) =>
        detail.tasks.some(
          (task) =>
            task.id === agent.currentTaskId &&
            task.title.toLowerCase().includes("cta"),
        ),
      );

      const apiAgent = detail.agents.find((agent) =>
        detail.tasks.some(
          (task) =>
            task.id === agent.currentTaskId &&
            task.title.toLowerCase().includes("api summary"),
        ),
      );
      const opsAgent = detail.agents.find((agent) =>
        detail.tasks.some(
          (task) =>
            task.id === agent.currentTaskId &&
            task.title.toLowerCase().includes("ops deployment"),
        ),
      );
      const artifactSources = [
        writerAgent?.worktreePath
          ? {
              source: join(writerAgent.worktreePath, "notes", "changelog.txt"),
              destination: join(repoRoot, "notes", "changelog.txt"),
            }
          : null,
        alphaHeroAgent?.worktreePath
          ? {
              source: join(
                alphaHeroAgent.worktreePath,
                "stories",
                "story-alpha-hero.txt",
              ),
              destination: join(repoRoot, "stories", "story-alpha-hero.txt"),
            }
          : null,
        alphaCtaAgent?.worktreePath
          ? {
              source: join(
                alphaCtaAgent.worktreePath,
                "stories",
              ...
              destination: join(repoRoot, "stories", "story-alpha-cta.txt"),
            }
          : null,
        apiAgent?.worktreePath
          ? {
              source: join(apiAgent.worktreePath, "docs", "api-summary.txt"),
              destination: join(repoRoot, "docs", "api-summary.txt"),
            }
          : null,
        opsAgent?.worktreePath
          ? {
              source: join(opsAgent.worktreePath, "ops", "deployment.txt"),
              destination: join(repoRoot, "ops", "deployment.txt"),
            }
          : null,
      ].filter((value): value is { source: string; destination: string } =>
        Boolean(value),
      );
      ...
      await runCommand("git", [
        ...
        "reset",
        "--hard",
        `origin/${handoffRepo.defaultBranch}`,
      ]);
      ...
      const { stdout: prUrl } = await runCommand(
        "gh",
        [
          "pr",
          "create",
      ...
      await api<RunDetail>(
        ...
        authToken,
        "POST",
        `/api/v1/runs/${run.id}/publish-branch`,
        {
          branchName,
      ...
      await api<RunDetail>(
        ...
        authToken,
        "POST",
        `/api/v1/runs/${run.id}/pull-request-handoff`,
        {
      ...
      const handoffRun = await api<RunDetail>(
        baseUrl,
        authToken,
        "GET",
        `/api/v1/runs/${run.id}`,
      ...
      if (handoffRun.handoffStatus !== "pr_open") {
        throw new Error(
          `Proof failed: expected handoffStatus pr_open, saw ${handoffRun.handoffStatus}`,
        );
      }
      ...
      const workerNodes = await api<WorkerNode[]>(
        baseUrl,
        authToken,
        "GET",
        "/api/v1/worker-nodes",
        ...
        JSON.stringify(
          {
            ...
            workerNodes: workerNodes.map((node) => ({
              ...
              status: node.status,
              activeClaims:
                (node.metadata as Record<string, unknown>).activeClaims ?? null,
              queueDepth:
                (node.metadata as Record<string, unknown>).queueDepth ?? null,
            })),
            messageCounts: {
    ...
    if (runDetail.status === "failed" || runDetail.status === "cancelled") {
      throw new Error(
        `Run entered terminal failure state: ${runDetail.status}`,
      );
    }
  ...
  throw new Error(
    "Timed out waiting for hosted coordination proof run to complete",
  );
}
