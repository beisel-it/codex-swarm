...
describe("project helpers", () => {
  ...
  it("derives project and ad hoc groupings from repository ownership", () => {
    ...
    expect(
      summaries[0].repositories.map((repository) => repository.id),
    ).toEqual(["repo-app"]);
    expect(summaries[0].runs.map((run) => run.id)).toEqual([
      "run-latest",
      "run-complete",
    ]);
    expect(summaries[0].activeRuns.map((run) => run.id)).toEqual([
      "run-latest",
    ]);
    expect(summaries[0].completedRuns).toBe(1);
    expect(summaries[0].lastRun?.id).toBe("run-latest");

    expect(adHoc.repositories.map((repository) => repository.id)).toEqual([
      "repo-docs",
    ]);
