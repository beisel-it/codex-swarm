    ...
    <section className="panel task-dag-shell">
      <div className="section-header task-dag-section-header">
        <div>
          ...
          {model.state === "ready" ? (
            <p className="task-dag-helper">{model.helperText}</p>
          ) : null}
        </div>
        <div
          className="task-dag-chip-row"
          aria-label="Dependency graph summary"
        >
      ...
      <div className="task-dag-legend" aria-label="Dependency graph legend">
        <span>
          <i className="task-dag-swatch is-root" />
          Root
        </span>
        <span>
          <i className="task-dag-swatch is-blocked" />
          Blocked
        </span>
        <span>
          <i className="task-dag-swatch is-selected" />
          Selected
        </span>
        <span>
          <i className="task-dag-swatch is-related" />
          Adjacent
        </span>
      </div>
      {model.state === "loading" ? (
      ...
      {model.state === "ready" ? (
        <>
          <div
            className="task-dag-viewport"
            role="group"
            aria-label="Task dependency graph"
          >
            <div
              className="task-dag-canvas"
              style={{ width: `${model.width}px`, height: `${model.height}px` }}
            >
              <svg
                className="task-dag-svg"
                width={model.width}
                height={model.height}
              ...
              {model.nodes.map((node) => (
                <button
                  ...
                  onKeyDown={(event) =>
                    handleNodeKeyDown(event, onSelectTask, node.taskId)
                  }
                ...
                >
                  <div className="task-dag-node-topline">
                    <span
                      className={`tone-chip tone-${toneByTaskId?.get(node.taskId) ?? "active"}`}
                    >
                  ...
                  <span className="task-dag-node-meta">
                    {node.blockedByTaskIds.length > 0
                      ? `blocked by ${node.blockedByTaskIds.length}`
                      : `${node.dependencyIds.length} deps`}
                  </span>
                </button>
              ))}
            </div>
