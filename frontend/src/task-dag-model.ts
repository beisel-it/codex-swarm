...
export function buildTaskDagRenderModel({
  ...
  for (const node of nodeMap.values()) {
    ...
    node.dependencyIds = [
      ...new Set(
        node.dependencyIds.filter((dependencyId) => nodeMap.has(dependencyId)),
      ),
    ];
  }
  ...
  const indegree = new Map(
    nodes.map((node) => [node.taskId, node.dependencyIds.length] as const),
  );
  const dependentsById = new Map(
    nodes.map(
      (node) => [node.taskId, [...node.dependentTaskIds].sort()] as const,
    ),
  );
  const rootTaskIds = (
    taskDag?.rootTaskIds.length
      ? taskDag.rootTaskIds
      : nodes
    ...
    (taskId, index, all) =>
      nodeMap.has(taskId) && all.indexOf(taskId) === index,
  );
  const blockedTaskIds = new Set(
    (taskDag?.blockedTaskIds.length
      ? taskDag.blockedTaskIds
      : nodes
          .filter((node) => node.blockedByTaskIds.length > 0)
          .map((node) => node.taskId)
  ...
  for (const taskId of orderedTaskIds) {
    ...
    const level = node.dependencyIds.reduce(
      (max, dependencyId) => Math.max(max, (levels.get(dependencyId) ?? 0) + 1),
      0,
    );
  ...
  const width =
    CANVAS_PADDING * 2 +
    (maxColumn + 1) * NODE_WIDTH +
    maxColumn * HORIZONTAL_GAP;
  const height =
    CANVAS_PADDING * 2 +
    maxRows * NODE_HEIGHT +
    Math.max(maxRows - 1, 0) * VERTICAL_GAP;

  const selectedId =
    selectedTaskId && nodeMap.has(selectedTaskId)
      ? selectedTaskId
      : (nodes[0]?.taskId ?? null);
  ...
  const positions = new Map(
    positionedNodes.map((node) => [node.taskId, node] as const),
  );
  const edges = [...edgeIds]
    .map((edgeId) => {
      ...
      return {
        ...
        relatedToSelection: Boolean(
          selectedId &&
          (sourceTaskId === selectedId || targetTaskId === selectedId),
        ),
        activelyBlocking:
          blockedTaskIds.has(targetTaskId) &&
          (nodeMap.get(targetTaskId)?.blockedByTaskIds.includes(sourceTaskId) ??
            false),
        path,
