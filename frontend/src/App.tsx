...
type Route =
  ...
  | {
      kind: "run";
      runId: string;
      section: "overview" | "board" | "lifecycle" | "review";
    };
...
function parseRoute(pathname: string): Route {
  ...
  if (segments[0] === "projects") {
    ...
    if (section === "repositories") {
      return {
        ...
        projectId,
        section,
        mode: segments[3] === "new" ? "new-repository" : undefined,
      };
    }
    if (section === "teams") {
      const mode =
    ...
    if (section === "runs") {
      return {
        ...
        projectId,
        section,
        mode: segments[3] === "new" ? "new-run" : undefined,
      };
    }
    if (section === "settings") {
  ...
  if (segments[0] === "adhoc-runs") {
    return {
      kind: "adhoc-runs",
      mode: segments[1] === "new" ? "new" : undefined,
    };
  }
...
async function refreshRuntimeConfig() {
  ...
  const match = script.match(
    /window\.__CODEX_SWARM_CONFIG__\s*=\s*(\{[\s\S]*\})\s*;?\s*$/,
  );
  if (!match) {
    throw new Error(
      "Runtime config refresh failed: invalid runtime-config.js payload",
    );
  }
...
async function loadProjectTeams(projectId?: string) {
  ...
  return requestJson<ProjectTeamDetail[]>(
    `/api/v1/project-teams${suffix}`,
  ).catch(() => []);
}
...
async function updateProjectTeam(
  ...
  input: ProjectTeamUpdateInput,
) {
  return requestJson<ProjectTeamDetail>(
    `/api/v1/project-teams/${encodeURIComponent(projectTeamId)}`,
    {
      method: "PATCH",
      body: JSON.stringify(input),
    },
...
async function loadRunEvents(runId: string) {
  return requestJson<ControlPlaneEvent[]>(
...
async function loadRunApprovals(runId: string) {
  return requestJson<Approval[]>(
    `/api/v1/approvals?runId=${encodeURIComponent(runId)}`,
  ).catch(() => []);
}

async function loadRunValidations(runId: string) {
  return requestJson<Validation[]>(
    `/api/v1/validations?runId=${encodeURIComponent(runId)}`,
  ).catch(() => []);
}

async function loadRunArtifacts(runId: string) {
  return requestJson<Artifact[]>(
    `/api/v1/artifacts?runId=${encodeURIComponent(runId)}`,
  ).catch(() => []);
}

async function loadRunMessages(runId: string) {
  return requestJson<Message[]>(
    `/api/v1/messages?runId=${encodeURIComponent(runId)}`,
  ).catch(() => []);
}

async function loadSessionTranscript(sessionId: string) {
  return requestJson<SessionTranscriptEntry[]>(
    `/api/v1/sessions/${encodeURIComponent(sessionId)}/transcript`,
  ).catch(() => []);
}

function mergeRunLiveRefreshRequests(
  left: RunLiveRefreshRequest | null,
  right: RunLiveRefreshRequest,
) {
  return {
...
function replaceRunScopedItems<T extends { runId: string }>(
  items: T[],
  runId: string,
  nextItems: T[],
) {
...
function appendLiveRunEvent(
  items: ControlPlaneEvent[],
  nextItem: ControlPlaneEvent,
) {
...
function refreshRequestForControlPlaneEvent(
  event: ControlPlaneEvent,
): RunLiveRefreshRequest {
...
async function openRunStream(
  ...
  const response = await fetch(
    buildApiUrl(`/api/v1/runs/${encodeURIComponent(runId)}/stream`),
    {
      headers,
      signal,
      cache: "no-store",
...
async function loadRepeatableRunDefinitions(repositoryId?: string) {
  const suffix = repositoryId
    ? `?repositoryId=${encodeURIComponent(repositoryId)}`
    : "";
  return requestJson<RepeatableRunDefinition[]>(
    `/api/v1/repeatable-runs${suffix}`,
  );
}

async function createRepeatableRunDefinition(
...
async function updateRepeatableRunDefinition(
  ...
  return requestJson<RepeatableRunDefinition>(
    `/api/v1/repeatable-runs/${encodeURIComponent(definitionId)}`,
    {
      method: "PATCH",
      body: JSON.stringify(input),
    },
...
async function loadRepeatableRunTriggers(repositoryId?: string) {
  const suffix = repositoryId
    ? `?repositoryId=${encodeURIComponent(repositoryId)}`
    : "";
  return requestJson<RepeatableRunTrigger[]>(
    `/api/v1/repeatable-run-triggers${suffix}`,
  );
}

async function createRepeatableRunTrigger(
  input: RepeatableRunTriggerCreateInput,
) {
...
async function updateRepeatableRunTrigger(
  ...
  return requestJson<RepeatableRunTrigger>(
    `/api/v1/repeatable-run-triggers/${encodeURIComponent(triggerId)}`,
    {
      method: "PATCH",
      body: JSON.stringify(input),
    },
...
async function loadExternalEventReceipts(repositoryId?: string) {
  const suffix = repositoryId
    ? `?repositoryId=${encodeURIComponent(repositoryId)}`
    : "";
  return requestJson<ExternalEventReceipt[]>(
    `/api/v1/external-event-receipts${suffix}`,
  );
}
...
async function loadGovernanceReport() {
  return requestJson<GovernanceAdminReport>(
    "/api/v1/admin/governance-report",
  ).catch(() => null);
}
...
async function loadSwarmData(): Promise<SwarmData> {
  try {
    ...
    const workerNodes = await requestJson<WorkerNode[]>(
      "/api/v1/worker-nodes",
    ).catch(() => []);
    ...
    return {
      ...
      runTaskDags: Object.fromEntries(
        details.map((detail) => [detail.id, detail.taskDag ?? null]),
      ),
      workerNodes,
      approvals,
      validations,
...
function buildTaskPresentation(
  ...
  const reworkTasks = allTasks.filter(
    (candidate) =>
  ...
  const isLegacy =
    !hasDefinitionOfDone && task.verificationStatus === "not_required";
  ...
  if (isLegacy) {
    ...
    (task.status === "awaiting_review" && task.verificationStatus === "pending")
  ) {
  ...
  const verificationCopyMap: Record<VerificationViewState, string> = {
    ...
    verification_running:
      "Verifier is checking delivered work against the definition of done.",
    ...
    rework_requested:
      "Leader opened follow-up work from verifier change requests.",
  ...
  const verificationSubtitleMap: Record<VerificationViewState, string> = {
    legacy:
      "Legacy task metadata remains readable, but automatic verification does not apply.",
  ...
  return {
    ...
    verifierLabel: task.verifierAgentId
      ...
      : verificationState === "awaiting_verification" ||
        ...
        : "Not assigned",
    latestSummary:
      task.latestVerificationSummary?.trim() ||
      (verificationState === "legacy"
...
function App() {
    ...
    members: [
      {
        ...
        role: "tech-lead",
        profile: "leader",
        responsibility: "Own sequencing, planning, and run closure.",
      },
      {
        name: "Implementer",
        role: "implementer",
        profile: "implementer",
        responsibility: "Implement the assigned slice.",
      },
    ] as ProjectTeamMemberDraft[],
  });
  const [projectTeamImportForm, setProjectTeamImportForm] = useState({
    blueprintId: "",
    name: "",
    description: "",
  });
  ...
  const [reviewFilter, setReviewFilter] =
    useState<ReviewQueueFilter>("awaiting");
  ...
  const queueRunLiveRefreshRef = useRef<
    (runId: string, request: RunLiveRefreshRequest) => void
  >(() => undefined);
  ...
  useEffect(() => {
    const projectTeam = data.projectTeams.find(
      (item) => item.id === runForm.projectTeamId,
    );
    if (!projectTeam) {
      return;
    }
    setRunForm((current) => ({
      ...current,
      concurrencyCap:
        current.concurrencyCap === String(projectTeam.concurrencyCap)
  ...
  const selectedProject =
    route.kind === "project"
      ? (projectSummaries.find((item) => item.project.id === route.projectId) ??
        null)
      : null;
  const selectedProjectTeams = useMemo(
    () =>
      selectedProject
        ? data.projectTeams.filter(
            (projectTeam) =>
              projectTeam.projectId === selectedProject.project.id,
          )
        : [],
    ...
    () =>
      selectedProject
        ? data.repositories.filter((repository) =>
            selectedProject.project.repositoryIds.includes(repository.id),
          )
        : [],
  ...
  const isSelectedRunStartPending = selectedRun
    ? pendingRunStarts.includes(selectedRun.id)
    : false;
  ...
  const selectedRunProject = selectedRun
    ? (projectSummaries.find(
        (summary) => summary.project.id === (selectedRun.projectId ?? null),
      ) ?? null)
    : null;
  const selectedProjectRunsFull = useMemo(
    () =>
      selectedProject
        ? data.runs.filter(
            (run) => run.projectId === selectedProject.project.id,
          )
        : [],
    [data.runs, selectedProject],
  );
  const isProjectRunCreate =
    route.kind === "project" &&
    route.section === "runs" &&
    route.mode === "new-run";
  const newRunRepositories = isProjectRunCreate
    ? selectedProjectRepositories
    : adHocWorkspace.repositories;
  const shouldShowRunRepositoryPicker = isProjectRunCreate
    ? newRunRepositories.length > 1
    : true;
  const runTasks = useMemo(
    () =>
      selectedRun
        ? data.tasks.filter((task) => task.runId === selectedRun.id)
        : [],
  ...
  const runApprovals = useMemo(
    () =>
      selectedRun
        ? data.approvals.filter((approval) => approval.runId === selectedRun.id)
        : [],
    ...
    () =>
      selectedRun
        ? data.validations.filter(
            (validation) => validation.runId === selectedRun.id,
          )
        : [],
  ...
  const runArtifacts = useMemo(
    () =>
      selectedRun
        ? data.artifacts.filter((artifact) => artifact.runId === selectedRun.id)
        : [],
  ...
  const runSessions = useMemo(
    () =>
      selectedRun
        ? data.sessions.filter((session) => session.runId === selectedRun.id)
        : [],
  ...
  const runAgents = useMemo(
    () =>
      selectedRun
        ? data.agents.filter((agent) => agent.runId === selectedRun.id)
        : [],
      ...
      new Map(
        runTasks.map(
          (task) =>
            [
              task.id,
              buildTaskPresentation(task, runTasks, runAgentsById),
            ] as const,
        ),
      ),
    [runAgentsById, runTasks],
  );
  const selectedRunTaskDag = selectedRun
    ? (data.runTaskDags[selectedRun.id] ?? null)
    : null;
  ...
  const dagToneByTaskId = useMemo(
    () =>
      new Map<string, string>(
        Array.from(runTaskPresentations.entries(), ([taskId, presentation]) => [
          taskId,
          presentation.primaryStatusTone,
        ]),
      ),
        ...
        await openRunStream(
          ...
          (frame) => {
            ...
            try {
              ...
              queueRunLiveRefreshRef.current(
                runId,
                refreshRequestForControlPlaneEvent(nextEvent),
              );
            } catch (error) {
              console.error(
                "[run-stream] failed to parse control plane event",
                error,
              );
            }
          },
  ...
  useEffect(() => {
    const artifact =
      runArtifacts.find((item) => item.kind === "diff") ??
      runArtifacts[0] ??
      null;
  ...
  useEffect(() => {
    if (newRunRepositories.length === 1) {
      ...
      setRunForm((current) =>
        current.repositoryId === onlyRepository.id
          ? current
          : { ...current, repositoryId: onlyRepository.id },
      );
      return;
    ...
    setRunForm((current) => {
      ...
      const stillVisible = newRunRepositories.some(
        (repository) => repository.id === current.repositoryId,
      );
  ...
  useEffect(() => {
    ...
    if (selectedProjectTeams.length === 1) {
      ...
      setRunForm((current) =>
        current.projectTeamId === onlyTeam.id
          ? current
          : { ...current, projectTeamId: onlyTeam.id },
      );
      return;
    ...
    setRunForm((current) => {
      ...
      const stillVisible = selectedProjectTeams.some(
        (projectTeam) => projectTeam.id === current.projectTeamId,
      );
  ...
  useEffect(() => {
    ...
    const activePendingRunIds = new Set(
      data.runs.filter((run) => run.status === "pending").map((run) => run.id),
    );
    setPendingRunStarts((current) =>
      current.filter((runId) => activePendingRunIds.has(runId)),
    );
  ...
  async function refreshRunSlices(
    runId: string,
    request: RunLiveRefreshRequest,
  ) {
    const [
      nextRunDetail,
      nextApprovals,
    ...
    setData((current) => {
      ...
      if (nextRunDetail) {
        const { tasks, agents, sessions, taskDag, ...runSummary } =
          nextRunDetail;
        nextData = {
          ...nextData,
          runs: nextData.runs.map((run) =>
            run.id === runId
              ? {
                  ...run,
                  ...runSummary,
      ...
      if (nextApprovals) {
        nextData = {
          ...nextData,
          approvals: replaceRunScopedItems(
            nextData.approvals,
            runId,
            nextApprovals,
          ),
      ...
      if (nextValidations) {
        nextData = {
          ...nextData,
          validations: replaceRunScopedItems(
            nextData.validations,
            runId,
            nextValidations,
          ),
        };
      ...
      if (nextArtifacts) {
        nextData = {
          ...nextData,
          artifacts: replaceRunScopedItems(
            nextData.artifacts,
            runId,
            nextArtifacts,
          ),
      ...
      if (nextMessages) {
        nextData = {
          ...nextData,
          messages: replaceRunScopedItems(
            nextData.messages,
            runId,
            nextMessages,
          ),
  ...
  function queueRunLiveRefresh(runId: string, request: RunLiveRefreshRequest) {
    ...
    refreshState.pendingByRunId.set(
      runId,
      mergeRunLiveRefreshRequests(
        refreshState.pendingByRunId.get(runId) ?? null,
        request,
      ),
    );
    ...
    void (async () => {
      try {
        while (liveRunRefreshStateRef.current.pendingByRunId.size > 0) {
          const nextEntry = liveRunRefreshStateRef.current.pendingByRunId
            .entries()
            .next().value as [string, RunLiveRefreshRequest] | undefined;
  ...
  function handleSidebarResizeStart(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    function onPointerMove(moveEvent: PointerEvent) {
      ...
      const nextWidth = Math.min(
        SIDEBAR_MAX_WIDTH,
        Math.max(SIDEBAR_MIN_WIDTH, initialWidth + delta),
      );
  ...
  async function handleCreateProject(event: FormEvent) {
    ...
    try {
      ...
      if (projectForm.repositoryIds.length > 0) {
        await Promise.all(
          projectForm.repositoryIds.map((repositoryId) =>
            updateRepository(repositoryId, { projectId: project.id }),
          ),
        );
      }
      setProjects((current) =>
  ...
  function patchProjectTeamMember(
    index: number,
    patch: Partial<ProjectTeamMemberDraft>,
  ) {
      ...
      if (editingProjectTeamId) {
        await updateProjectTeam(editingProjectTeamId, {
          ...
          concurrencyCap: Math.max(
            1,
            Number.parseInt(projectTeamForm.concurrencyCap, 10) || 1,
          ),
          members,
        });
          ...
          current.map((project) =>
            project.id === selectedProject.project.id
              ? {
                  ...project,
                  repositoryIds: [
                    ...new Set([...project.repositoryIds, repository.id]),
                  ],
  ...
  async function handleCreateRun(event: FormEvent) {
    ...
    try {
      const run = await createRun({
        ...
        projectId: isProjectRunCreate
          ? (selectedProject?.project.id ?? null)
          : null,
        projectTeamId: isProjectRunCreate
          ? runForm.projectTeamId || null
          : null,
        goal: runForm.goal.trim(),
        branchName: runForm.branchName.trim() || null,
        concurrencyCap: Math.max(
          1,
          Number.parseInt(runForm.concurrencyCap, 10) || 1,
        ),
        handoff: {
          mode: "manual",
          provider: null,
  ...
  async function handleStartRun() {
    ...
    try {
      ...
      setPendingRunStarts((current) =>
        current.filter((pendingRunId) => pendingRunId !== runId),
      );
  ...
  const filteredProjects = projectSummaries.filter((summary) => {
    const haystack =
      `${summary.project.name} ${summary.project.summary}`.toLowerCase();
  ...
  const filteredAdHocRunsFull = data.runs.filter((run) => {
    if (
      !adHocWorkspace.repositories.some(
        (repository) => repository.id === run.repositoryId,
      )
    ) {
      return false;
    }
    const repository = data.repositories.find(
      (item) => item.id === run.repositoryId,
    );
    const haystack =
      `${run.goal} ${repository?.name ?? ""} ${run.status}`.toLowerCase();
    return haystack.includes(runQuery.toLowerCase());
  });

  const filteredProjectRepos = selectedProjectRepositories.filter(
    (repository) => {
      const haystack =
        `${repository.name} ${repository.url} ${repository.provider}`.toLowerCase();
  ...
  const completedTasks = runTasks.filter((task) => {
    ...
    return (
      presentation?.verificationState === "verified_complete" ||
      (task.status === "completed" && presentation?.isLegacy)
    );
  });
  const waitingTasks = activeTasks.filter((task) => {
    const presentation = runTaskPresentations.get(task.id);
    return (
      presentation?.verificationState === "awaiting_verification" ||
      presentation?.verificationState === "verification_running"
    );
  });
  ...
  const inFlightTasks = activeTasks.filter((task) => {
    ...
    return (
      ...
      presentation?.verificationState === "not_requested"
    );
  });
  ...
  const filteredVerificationTasks = verificationTasks.filter((task) => {
    const presentation = runTaskPresentations.get(task.id);
    return presentation
      ? matchesReviewFilter(presentation, reviewFilter)
      : false;
  });
  const selectedTask =
    runTasks.find((task) => task.id === selectedTaskId) ?? runTasks[0] ?? null;
  const selectedVerificationTask =
    filteredVerificationTasks.find((task) => task.id === selectedTaskId) ??
    filteredVerificationTasks[0] ??
    null;
    ...
    <div className="app-shell">
      <header className="primary-header">
        ...
        <nav className="primary-nav" aria-label="Global">
          ...
          <button
            type="button"
            className={route.kind === "adhoc-runs" ? "is-active" : ""}
            onClick={() => navigate("/adhoc-runs")}
          >
            Ad-Hoc Runs
          </button>
          <button
            type="button"
            className={route.kind === "settings" ? "is-active" : ""}
        ...
        <div className="header-meta">
          <span className="status-chip">
            {data.workerNodes.filter((node) => node.status === "online").length}{" "}
            Online Nodes
          </span>
          <span className="status-chip">
            {data.source === "api" ? "Live API" : "Mock API"}
          </span>
          <label className="theme-picker">
            ...
            <select
              value={activeTheme}
              onChange={(event) =>
                setActiveTheme(event.target.value as typeof activeTheme)
              }
            >
              {themes.map((theme) => (
        ...
        <section className="context-header">
          <div>
            ...
            <p>
              {selectedProject.project.summary ||
                "Project context for teams, repositories, runs, automation, and policy defaults."}
            </p>
          </div>
          <nav className="context-tabs" aria-label="Project sections">
            {[
              ...
              [
                "Repositories",
                `/projects/${selectedProject.project.id}/repositories`,
              ],
              ["Runs", `/projects/${selectedProject.project.id}/runs`],
              [
                "Automation",
                `/projects/${selectedProject.project.id}/automation`,
              ],
      ...
      {route.kind === "run" && selectedRun ? (
        <section className="context-header run-context">
          <div>
            <p className="eyebrow">
              {selectedRunProject
                ? `Projects / ${selectedRunProject.project.name}`
                : "Operations"}{" "}
              / {selectedRepository?.name ?? "Run"}
            </p>
            <div className="run-title-row">
              <span className="run-id-badge">
                run {shortRunId(selectedRun.id)}
              </span>
              <h1 className="run-title-preview" title={selectedRun.goal}>
                {selectedRun.goal}
              </h1>
            </div>
            <div className="inline-facts">
              ...
              <span
                className={`tone-chip tone-${toneForStatus(selectedRun.status)}`}
              >
                {formatLabel(selectedRun.status)}
              </span>
              <span>
                {selectedRun.pullRequestNumber
                  ? `PR #${selectedRun.pullRequestNumber}`
            ...
            <>
              <div className="sidebar-section">
                ...
                <div className="saved-views">
                  <button
                    type="button"
                    className={sidebarPillClassName(projectView === "all")}
                  ...
                  <button
                    type="button"
                    className={sidebarPillClassName(projectView === "recent")}
                    onClick={() => setProjectView("recent")}
                  >
                    Recently active
                  </button>
                  <button
                    type="button"
                    className={sidebarPillClassName(
                      projectView === "needs-setup",
                    )}
                    onClick={() => setProjectView("needs-setup")}
                  >
                    Needs setup
                  </button>
                </div>
              ...
              <>
                ...
                <div className="sidebar-section">
                  <button
                    ...
                    onClick={() =>
                      navigate(
                        `/projects/${selectedProject.project.id}/repositories/new`,
                      )
                    }
                  >
                    New Repo
                ...
                <div className="sidebar-section">
                  ...
                  <div className="sidebar-button-stack">
                    <button
                      ...
                      onClick={() =>
                        navigate(
                          `/projects/${selectedProject.project.id}/teams/new`,
                        )
                      }
                    >
                      New team
                    ...
                    <button
                      ...
                      onClick={() =>
                        navigate(
                          `/projects/${selectedProject.project.id}/teams/import`,
                        )
                      }
                    >
                      Import blueprint
                ...
                <div className="sidebar-section">
                  <div className="sidebar-button-stack">
                    <button
                      ...
                      onClick={() =>
                        navigate(
                          `/projects/${selectedProject.project.id}/runs/new`,
                        )
                      }
                    >
                      New Run
              ...
              <>
                <div className="sidebar-section">
                  ...
                  <div className="sidebar-button-stack">
                    <button
                      type="button"
                      className={sidebarPillClassName(
                        route.mode === "new-repeatable-run",
                      )}
                      onClick={() =>
                        navigate(
                          `/projects/${selectedProject.project.id}/automation/repeatable-runs/new`,
                        )
                      }
                    >
                      New repeatable run
                    ...
                    <button
                      type="button"
                      className={sidebarPillClassName(
                        route.mode === "new-webhook",
                      )}
                      onClick={() =>
                        navigate(
                          `/projects/${selectedProject.project.id}/automation/webhooks/new`,
                        )
                      }
                    >
                      New webhook trigger
                ...
                <div className="sidebar-section">
                  <label className="field">
                    ...
                    <select
                      value={automationRepositoryId}
                      onChange={(event) =>
                        setAutomationRepositoryId(event.target.value)
                      }
                    >
              ...
              <div className="sidebar-section">
                ...
                <label className="field">
                  ...
                  <input
                    value={runQuery}
                    onChange={(event) => setRunQuery(event.target.value)}
                    placeholder="Search ad-hoc runs"
                  />
                </label>
              </div>
              <div className="sidebar-section">
                <div className="sidebar-button-stack">
                  <button
                    type="button"
                    className="sidebar-primary-action"
                    onClick={() => navigate("/adhoc-runs/new")}
                  >
                    New Ad-Hoc Run
                  </button>
                </div>
            ...
            <>
                ...
                <div className="sidebar-button-stack">
                  ...
                  {isSelectedRunStartPending ? (
                    <span className="sidebar-action-note">
                      Start requested. Waiting for the scheduler.
                    </span>
                  ) : null}
                  <button
                    type="button"
                    ...
                    onClick={() => navigate(`/runs/${selectedRun.id}/overview`)}
                  >
                    Overview
                  </button>
                  <button
                    type="button"
                    className={sidebarPillClassName(route.section === "board")}
                    onClick={() => navigate(`/runs/${selectedRun.id}/board`)}
                  >
                    Board
                  </button>
                  <button
                    ...
                    onClick={() =>
                      navigate(`/runs/${selectedRun.id}/lifecycle`)
                    }
                  >
                    Lifecycle
                  </button>
                  <button
                    type="button"
                    className={sidebarPillClassName(route.section === "review")}
                    onClick={() => navigate(`/runs/${selectedRun.id}/review`)}
                  >
                    Review
                  </button>
                </div>
              ...
              <div className="sidebar-section">
                ...
                {selectedRun.planArtifactPath ? (
                  ...
                  <span className="muted-line">
                    No plan artifact published yet.
                  </span>
                )}
              </div>
            </>
            ...
            <>
              <div className="sidebar-section">
                ...
                <div className="saved-views">
                  ...
                  <button
                    type="button"
                    className={sidebarPillClassName(settingsScope === "policy")}
                    onClick={() => setSettingsScope("policy")}
                  >
                    Policy
                  </button>
                  <button
                    ...
                    onClick={() => setSettingsScope("provider")}
                  >
                    Provider
                  </button>
                </div>
        ...
        <main className="content">
          {loading ? (
            <section className="panel">
              <div className="compact-empty">Loading workspace…</div>
            </section>
          ) : null}
          {refreshError ? (
            <section className="panel">
              <div className="compact-empty">{refreshError}</div>
            </section>
          ) : null}
              ...
              <section className="panel">
                ...
                <div className="stats-row">
                  ...
                  <article className="stat-card">
                    ...
                    <strong>
                      {projectSummaries[0]?.lastRun
                        ? formatDate(projectSummaries[0].lastRun.updatedAt)
                        : "n/a"}
                    </strong>
                  </article>
                </div>
                  ...
                  <tbody>
                    {filteredProjects.map((summary) => (
                      <tr key={summary.project.id}>
                        ...
                        <td>
                          {summary.activeRuns.length > 0 ? "Active" : "Quiet"}
                        </td>
                ...
                <fieldset className="repo-picker">
                  ...
                  {data.repositories.map((repository) => (
                    <label key={repository.id} className="checkbox-row">
                      <input
                        type="checkbox"
                        checked={projectForm.repositoryIds.includes(
                          repository.id,
                        )}
                        onChange={() =>
                          setProjectForm((current) => ({
                            ...current,
                            repositoryIds: current.repositoryIds.includes(
                              repository.id,
                            )
              ...
              <>
                <section className="panel">
                  <div className="stats-row">
                    <article className="stat-card">
                      <span>Teams</span>
                      <strong>{selectedProjectTeams.length}</strong>
                    </article>
                    <article className="stat-card">
                      <span>Repositories</span>
                    ...
                    <article className="stat-card">
                      <span>Runs</span>
                      <strong>{selectedProject.runs.length}</strong>
                    </article>
                    <article className="stat-card">
                      <span>Active runs</span>
                      <strong>{selectedProject.activeRuns.length}</strong>
                    </article>
                  </div>
                </section>
                <section className="panel split-panel">
                  <article className="surface-card">
                    ...
                    <p>
                      {selectedProject.project.summary ||
                        "This project groups teams, repositories, run history, and automation rules."}
                    </p>
                  </article>
                  <article className="surface-card">
                    ...
                    <p>
                      {selectedProjectTeams.length > 0
                        ? `${selectedProjectTeams.length} project team${selectedProjectTeams.length === 1 ? "" : "s"} are available for runs and automation.`
                        : "Create or import a project team before planning project runs."}
                    </p>
                  </article>
                </section>
              </>
              ...
              <>
                  ...
                  <section className="panel form-panel">
                    <div className="section-header">
                      <div>
                        ...
                        <h2>
                          {editingProjectTeamId ? "Edit team" : "New team"}
                        </h2>
                      </div>
                    </div>
                    <form
                      className="stack-form"
                      onSubmit={(event) => void handleSubmitProjectTeam(event)}
                    >
                      <label className="field">
                        <span>Name</span>
                        <input
                          value={projectTeamForm.name}
                          onChange={(event) =>
                            setProjectTeamForm((current) => ({
                              ...current,
                              name: event.target.value,
                      ...
                      <label className="field">
                        ...
                        <textarea
                          rows={3}
                          value={projectTeamForm.description}
                          onChange={(event) =>
                            setProjectTeamForm((current) => ({
                              ...current,
                      ...
                      <label className="field">
                        ...
                        <input
                          type="number"
                          min={1}
                          value={projectTeamForm.concurrencyCap}
                          onChange={(event) =>
                            setProjectTeamForm((current) => ({
                              ...current,
                              concurrencyCap: event.target.value,
                            }))
                          }
                        />
                      </label>
                      ...
                      <div className="action-row">
                        <button
                          ...
                          disabled={busy}
                        >
                          {editingProjectTeamId ? "Save team" : "Create team"}
                        </button>
                        {editingProjectTeamId ? (
                          <button
                            type="button"
                            className="table-action"
                            onClick={resetProjectTeamForm}
                          >
                            Cancel
                          </button>
                        ) : null}
                ...
                {route.mode === "import-team" ? (
                  <section className="panel form-panel">
                    ...
                    <form
                      className="stack-form"
                      onSubmit={(event) => void handleImportProjectTeam(event)}
                    >
                      <label className="field">
                        <span>Blueprint</span>
                        <select
                          value={projectTeamImportForm.blueprintId}
                          onChange={(event) =>
                            setProjectTeamImportForm((current) => ({
                              ...current,
                      ...
                      <label className="field">
                        ...
                        <input
                          value={projectTeamImportForm.name}
                          onChange={(event) =>
                            setProjectTeamImportForm((current) => ({
                              ...current,
                              name: event.target.value,
                      ...
                      <label className="field">
                        ...
                        <textarea
                          rows={3}
                          value={projectTeamImportForm.description}
                          onChange={(event) =>
                            setProjectTeamImportForm((current) => ({
                              ...current,
                              description: event.target.value,
                      ...
                      <div className="action-row">
                        <button
                          type="submit"
                          className="action-button"
                          disabled={busy || !projectTeamImportForm.blueprintId}
                        >
                          Import team
                        </button>
                      </div>
                    ...
                    <tbody>
                      {selectedProjectTeams.map((projectTeam) => (
                        <tr key={projectTeam.id}>
                          ...
                          <td>
                            {projectTeam.sourceBlueprintId ??
                              projectTeam.sourceTemplateId ??
                              "Manual"}
                          </td>
                          <td>
                            <div className="action-row">
                                ...
                                onClick={() => {
                                  ...
                                  setProjectTeamForm({
                                    ...
                                    concurrencyCap: String(
                                      projectTeam.concurrencyCap,
                                    ),
                                    members: projectTeam.members.map(
                                      (member) => ({
                                        name: member.name,
                                        role: member.role,
                              ...
                              <button
                                ...
                                className="table-action table-action-danger"
                                onClick={() =>
                                  void handleDeleteProjectTeam(projectTeam.id)
                                }
                                disabled={busy}
                              >
                                Delete
                      ...
                      {selectedProjectTeams.length === 0 ? (
                        <tr>
                          <td colSpan={5}>
                            <div className="compact-empty">
                              No project teams configured yet.
                            </div>
                          </td>
                        </tr>
                      ) : null}
              ...
              <>
                {route.mode === "new-repository" ? (
                  <section className="panel form-panel">
                    ...
                    <form
                      className="stack-form"
                      onSubmit={(event) => void handleCreateRepository(event)}
                    >
                      <label className="field">
                        <span>Name</span>
                        <input
                ...
                {route.mode === "new-run" ? (
                  newRunRepositories.length === 0 ? (
                    ...
                    <section className="panel form-panel">
                      ...
                      <div className="compact-empty">
                        Create or import a project team before planning a
                        project run.{" "}
                        <button
                          type="button"
                          className="table-action"
                    ...
                    <section className="panel form-panel">
                      ...
                      <form
                        className="stack-form"
                        onSubmit={(event) => void handleCreateRun(event)}
                      >
                        {shouldShowRunRepositoryPicker ? (
                          <label className="field">
                            <span>Repository</span>
                            <select
                              value={runForm.repositoryId}
                              onChange={(event) =>
                                setRunForm((current) => ({
                                  ...current,
                        ...
                        <label className="field">
                          ...
                          <select
                            value={runForm.projectTeamId}
                            onChange={(event) =>
                              setRunForm((current) => ({
                                ...current,
                                projectTeamId: event.target.value,
                              }))
                            }
                          >
                      ...
                      {filteredProjectRuns.map((run) => {
                        ...
                        return (
                          <tr key={run.id}>
                            ...
                            <td>
                              <span
                                className={`tone-chip tone-${toneForStatus(run.status)}`}
                              >
          ...
          {!loading &&
            ...
            route.section === "automation" && (
              <section className="panel">
                ...
                <RepeatableRunsPanel
                  repositories={selectedProjectRepositories.map(
                    (repository) => ({
                      id: repository.id,
                  ...
                  definitions={data.repeatableRunDefinitions.filter(
                    (definition) =>
                      selectedProject.project.repositoryIds.includes(
            ...
            route.section === "settings" && (
              <section className="panel split-panel">
                <article className="surface-card">
                  ...
                  <p>
                    Use this area for project-owned branch defaults, provider
                    mapping, and policy profile decisions.
                  </p>
                  <ul className="plain-list">
                    <li>
                      Repositories linked: {selectedProject.repositories.length}
                    </li>
                    <li>
                      Latest run: {selectedProject.lastRun?.goal ?? "n/a"}
                    </li>
                    <li>
                      Suggested default branch:{" "}
                      {selectedProjectRepositories[0]?.defaultBranch ?? "main"}
                    </li>
                  </ul>
                </article>
                <article className="surface-card">
                  ...
                  <h3>Boundary</h3>
                  <p>
                    Global governance is handled in Settings. This page is
                    reserved for project-scoped configuration only.
                  </p>
                </article>
              </section>
            )}
              ...
              {route.mode === "new" ? (
                newRunRepositories.length === 0 ? (
                  <section className="panel form-panel">
                    ...
                    <div className="compact-empty">
                      No unassigned repositories are available for ad-hoc runs.
                    </div>
                  </section>
                ) : (
                  <section className="panel form-panel">
                  ...
                  <tbody>
                        ...
                        <tr key={run.id}>
                          ...
                          <td className="table-actions">
                            ...
                            <button
                              type="button"
                              className="table-action"
                              onClick={() => navigate(`/runs/${run.id}/board`)}
                            >
                              Board
                            </button>
                          </td>
                    ...
                    {filteredAdHocRunsFull.length === 0 ? (
                      <tr>
                        <td colSpan={6}>
                          <div className="compact-empty">
                            No ad-hoc runs match the current filter.
                          </div>
                        </td>
                      </tr>
                    ) : null}
            ...
            <>
              <section className="panel">
                <div className="stats-row">
                  <article className="stat-card">
                    <span>Workspace</span>
                    <strong>
                      {data.identity?.workspace.name ?? "Unavailable"}
                    </strong>
                  </article>
                  <article className="stat-card">
                    <span>Team</span>
                    <strong>{data.identity?.team.name ?? "Unavailable"}</strong>
                  </article>
                  <article className="stat-card">
                    <span>Pending approvals</span>
                    <strong>{data.governance?.approvals.pending ?? 0}</strong>
                  </article>
                  <article className="stat-card">
                    <span>Repositories</span>
                    <strong>{data.repositories.length}</strong>
              ...
              <section className="panel split-panel">
                ...
                <article className="surface-card">
                  ...
                  <ul className="plain-list">
                    <li>
                      Approval totals: {data.governance?.approvals.total ?? 0}
                    </li>
                    <li>
                      Secret source:{" "}
                      {data.governance?.secrets.sourceMode ?? "n/a"}
                    </li>
                    <li>
                      Allowed trust levels:{" "}
                      {data.governance?.secrets.allowedRepositoryTrustLevels.join(
                        ", ",
                      ) || "n/a"}
                    </li>
                  </ul>
              ...
              <section className="panel">
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {data.governance?.policies.repositoryProfiles.map(
                      (profile) => (
                      ...
                      <tr>
                        <td colSpan={3}>
                          <div className="compact-empty">
                            No governance policy data available.
                          </div>
                        </td>
                      </tr>
                    )}
          ...
          {!loading && route.kind === "run" && !selectedRun && (
            <section className="panel">
              <div className="compact-empty">Run not found.</div>
            </section>
          )}
              ...
              <>
                <section className="panel">
                  <div className="stats-row">
                    <article className="stat-card">
                      <span>Status</span>
                      <strong>{formatLabel(selectedRun.status)}</strong>
                    </article>
                    <article className="stat-card">
                ...
                <section className="panel split-panel">
                  <article className="surface-card">
                    ...
                    <h3>
                      {selectedRepository?.name ?? "Repository unavailable"}
                    </h3>
                    <ul className="plain-list">
                      ...
                      <li>
                        Published branch:{" "}
                        {selectedRun.publishedBranch ?? "Not published"}
                      </li>
                      <li>
                        Plan artifact:{" "}
                        {selectedRun.planArtifactPath ?? "Not published"}
                      </li>
                      <li>
                        Run kind:{" "}
                        {runKindLabel(selectedRunProject?.project.id ?? null)}
                      </li>
                    </ul>
                  </article>
                  <article className="surface-card">
                        ...
                        <div key={approval.id} className="compact-list-row">
                          ...
                          <strong>
                            {approval.status === "pending"
                              ? String(
                                  approval.requestedPayload?.summary ??
                                    "Awaiting decision",
                                )
                              : String(
                                  approval.resolutionPayload?.feedback ??
                                    "Resolved",
                                )}
                          </strong>
                        </div>
                    ...
                    <details className="secondary-panel">
                      ...
                      <div className="compact-list">
                        {runSessions.map((session) => {
                          const node = data.workerNodes.find(
                            (item) => item.id === session.workerNodeId,
                          );
                          return (
                            <div key={session.id} className="compact-list-row">
                    ...
                    <details className="secondary-panel">
                      ...
                      <div className="compact-list">
                        ...
                        {runValidations.length === 0 ? (
                          <div className="compact-empty">
                            No validation evidence published yet.
                          </div>
                        ) : null}
                      </div>
                    </details>
                  ...
                  <table className="data-table">
                    ...
                    <tbody>
                      {runTasks
                        ...
                        .sort(compareTasks)
                        .map((task) => {
                          const presentation = runTaskPresentations.get(
                            task.id,
                          );
                          if (!presentation) {
                            return null;
                          ...
                          return (
                            ...
                            >
                              ...
                              <td>
                                <span
                                  className={`tone-chip tone-${presentation.primaryStatusTone}`}
                                >
                              ...
                              <td>
                                <span
                                  className={`tone-chip tone-${presentation.verificationTone}`}
                                >
                                  {presentation.verificationLabel}
                                </span>
                              </td>
                              <td>
                                {task.latestVerificationChangeRequests.length}
                              </td>
                ...
                <section className="panel split-panel">
                  ...
                  <article className="surface-card">
                    ...
                    <div className="compact-list">
                      ...
                      {selectedSessionId && transcript.length === 0 ? (
                        <div className="compact-empty">
              ...
              <>
                <section className="panel">
                  ...
                  <div className="filter-row">
                    {(
                      ...
                      <button
                        key={value}
                        type="button"
                        className={sidebarPillClassName(reviewFilter === value)}
                  ...
                  {filteredVerificationTasks.length === 0 ? (
                    <div className="compact-empty">
                      No tasks currently match this verification filter.
                    </div>
                  ) : (
                        ...
                        {filteredVerificationTasks.map((task) => {
                          ...
                          return (
                            <button
                              key={task.id}
                              type="button"
                              className={`review-item ${task.id === selectedVerificationTask?.id ? "is-selected" : ""}`}
                              onClick={() => handleTaskSelection(task.id)}
                            >
                              <strong>{task.title}</strong>
                              <span
                ...
                <section className="panel split-panel">
                  <article className="surface-card">
                    ...
                    {runApprovals.length === 0 ? (
                      <div className="compact-empty">
                        No approvals returned for this run.
                      </div>
                    ) : (
                      <div className="review-layout">
                        <div className="review-list">
                          {runApprovals.map((approval) => (
                            <button
                              key={approval.id}
                              type="button"
                              className={`review-item ${approval.id === selectedApprovalId ? "is-selected" : ""}`}
                              onClick={() => setSelectedApprovalId(approval.id)}
                            >
                              <strong>{approval.kind}</strong>
                              <span
                                className={`tone-chip tone-${toneForStatus(approval.status)}`}
                              >
                        ...
                        <div className="review-detail">
                          <div className="surface-card">
                            ...
                            <p>
                              {String(
                                approvalDetail?.requestedPayload?.summary ??
                                  "No structured request summary attached.",
                              )}
                            </p>
                            <label className="field">
                              <span>Notes</span>
                  ...
                  <article className="surface-card">
                    ...
                    {runArtifacts.length === 0 ? (
                      ...
                      <>
                        ...
                        <div className="compact-list">
                          {artifactDetail?.diffSummary?.fileSummaries
                            .slice(0, 6)
                            .map((fileSummary) => (
                              <div
                                key={`${fileSummary.path}-${fileSummary.changeType}`}
                                className="compact-list-row"
                              >
                                <span>
                                  {formatLabel(fileSummary.changeType)}
                                </span>
                          ...
                          {!artifactDetail?.diffSummary?.fileSummaries
                            .length ? (
                            <div className="compact-empty">
                              No per-file diff summary available for this
                              artifact.
                            </div>
                          ) : null}
                        </div>
          ...
          return (
            ...
            >
              ...
              <div className="task-card-checklist">
                {presentation.hasDefinitionOfDone ? (
                  <>
                    ...
                    {task.definitionOfDone.length > 2 ? (
                      <span>
                        +{task.definitionOfDone.length - 2} more DoD items
                      </span>
                    ) : null}
                  </>
                ) : (
              ...
              <div className="inline-facts">
                ...
                {task.latestVerificationChangeRequests.length > 0 ? (
                  <span>
                    {task.latestVerificationChangeRequests.length} change
                    requests
                  </span>
                ) : null}
              </div>
...
function TaskDetailPanel({
  ...
  if (!task) {
    return (
      <article id="task-detail-panel" className="surface-card task-detail-card">
        ...
        <div className="compact-empty">
          Select a task to inspect its definition of done and verification
          state.
        </div>
      </article>
    );
  ...
  const relatedArtifacts = artifacts
    .filter((artifact) => artifact.runId === task.runId)
    .slice(0, 5);
    ...
    <article id="task-detail-panel" className="surface-card task-detail-card">
      ...
      <div className="task-detail-header">
        ...
        <div className="task-detail-chips">
          <span className={`tone-chip tone-${presentation.primaryStatusTone}`}>
            {formatLabel(task.status)}
          </span>
          <span className={`tone-chip tone-${presentation.verificationTone}`}>
            {presentation.verificationLabel}
          </span>
        </div>
      </div>
      ...
      <div className="detail-stack">
        ...
        <details
          className="secondary-panel"
          open={!presentation.hasDefinitionOfDone}
        >
          ...
          {task.acceptanceCriteria.length > 0 ? (
            ...
            <div className="compact-empty">
              No acceptance criteria were published for this task.
            </div>
          )}
        </details>
        {findings.length > 0 ||
              ...
              <>
                ...
                <ul className="plain-list">
                  {presentation.reworkTasks.map((reworkTask) => (
                    <li key={reworkTask.id}>
                      {reworkTask.title} ({formatLabel(reworkTask.status)})
                    </li>
                  ))}
                </ul>
              </>
        ...
        <section className="detail-block">
          ...
          <ul className="plain-list">
            ...
            {task.validationTemplates.length > 0 ? (
              <li>
                {task.validationTemplates.length} validation templates attached.
              </li>
            ) : null}
            {task.latestVerificationEvidence.length > 0 ? (
              <li>
                {task.latestVerificationEvidence.length} verification evidence
                references attached.
              </li>
            ) : null}
            {validations.length > 0 ? (
              <li>{validations.length} run validations available.</li>
            ) : null}
            {relatedArtifacts.length > 0 ? (
              <li>{relatedArtifacts.length} recent artifacts available.</li>
            ) : null}
          </ul>
        </section>
        <section className="detail-block">
          ...
          <div className="compact-list">
            ...
            {relatedEvents.length === 0 ? (
              <div className="compact-empty">
                No verification events have been published for this task yet.
              </div>
            ) : null}
          </div>
        </section>
