  ...
  return {
    ...
    props: {
      ...
      children: toArray(node.props.children).map((child) =>
        resolveTree(child as ReactNodeLike),
      ),
    },
  };
}
  ...
  it("derives a directional layout and selection relationships from task dependencies", () => {
    ...
    expect(
      rootNode && branchNode && leafNode
        ? rootNode.x < branchNode.x && branchNode.x < leafNode.x
        : false,
    ).toBe(true);
    expect(
      model.edges.some(
        (edge) =>
          edge.activelyBlocking &&
          edge.sourceTaskId === "task-root" &&
          edge.targetTaskId === "task-branch",
      ),
    ).toBe(true);
  });
...
describe("TaskDagGraphPanel", () => {
  it("wires node buttons to the shared task selection callback", () => {
    ...
    const nodeButton = buttons.find(
      (button) => button.props.title === "Branch task",
    );
