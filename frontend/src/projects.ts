  ...
  return normalizeProjects(projects)
    .map((project) => {
      const projectRepositories = project.repositoryIds
        ...
        .filter(
          (repository): repository is ProjectRepository =>
            repository !== undefined,
        );
      const repositoryIdSet = new Set(
        projectRepositories.map((repository) => repository.id),
      );
      const projectRuns = runs
        .filter(
          (run) =>
            run.projectId === project.id ||
            (!run.projectId && repositoryIdSet.has(run.repositoryId)),
        )
      ...
      return {
        ...
        completedRuns: projectRuns.filter((run) => run.status === "completed")
          .length,
    ...
    .sort((left, right) => {
      ...
      const rightTimestamp =
        right.lastRun?.updatedAt ?? right.project.updatedAt;
      return (
...
export function deriveAdHocWorkspace(
  ...
  const assignedRepositoryIds = new Set(
    normalizeProjects(projects).flatMap((project) => project.repositoryIds),
  );
  const adHocRepositories = repositories
    .filter((repository) => !assignedRepositoryIds.has(repository.id))
    .sort((left, right) => left.name.localeCompare(right.name));
  const adHocRepositoryIds = new Set(
    adHocRepositories.map((repository) => repository.id),
  );
  ...
  return [
    {
      ...
      summary:
        "Primary workspace for linked repos, active runs, and historical job context.",
