...
export type AgentObservabilityMode = 'session' | 'transcript_visibility' | 'unavailable'
...
export type AgentObservabilityInput = Partial<AgentObservability> | null | undefined
...
export function normalizeAgentObservability(input?: AgentObservabilityInput): AgentObservability {
  return {
...
function summarizeLiveSession(observability: AgentObservability, session: SessionLike | null, visibleTranscriptSession: SessionLike | null) {
  if (!session) {
    return 'Live session linkage is present, but the current session record has not hydrated yet.'
  }

  if (observability.visibleTranscriptSessionId && observability.visibleTranscriptSessionId !== session.id && visibleTranscriptSession) {
...
function summarizeFallback(observability: AgentObservability, visibleTranscriptSession: SessionLike | null) {
...
export function buildAgentTranscriptTargets(agents: AgentLike[], sessions: SessionLike[]): TranscriptAccessTarget[] {
  ...
  return agents.map((agent) => {
    ...
    const currentSession = observability.currentSessionId ? (sessionsById.get(observability.currentSessionId) ?? null) : null
    ...
    if (observability.mode === 'session' && observability.currentSessionId) {
      return {
        ...
        summaryDetail: summarizeLiveSession(observability, currentSession, visibleTranscriptSession),
        badgeLabel: 'Live transcript',
      }
    }

    if (observability.mode === 'transcript_visibility' && observability.visibleTranscriptSessionId) {
      return {
        agentId: agent.id,
        agentName: agent.name,
        ...
        summaryDetail: summarizeFallback(observability, visibleTranscriptSession),
...
export function chooseTranscriptSessionId(currentSessionId: string, targets: TranscriptAccessTarget[]): string {
  if (currentSessionId && targets.some((target) => target.sessionId === currentSessionId)) {
    return currentSessionId
  }

  return (
    targets.find((target) => target.mode === 'live_session')?.sessionId
    ?? targets.find((target) => target.mode === 'fallback_transcript')?.sessionId
    ?? ''
  )
}
