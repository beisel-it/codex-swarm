...
const captureRoot = resolve(
  process.env.TUI_CAPTURE_ROOT ?? ".ops/tui-captures",
);
const label =
  process.env.TUI_CAPTURE_LABEL ??
...
const captureSeconds = Number.parseInt(
  process.env.TUI_CAPTURE_SECONDS ?? "8",
  10,
);
...
if (
  ...
  console.error(
    "TUI_CAPTURE_COLUMNS must be >= 80 and TUI_CAPTURE_ROWS must be >= 24.",
  );
  process.exit(1);
}
...
const metadata = {
  ...
  envHints: {
    ...
    CODEX_SWARM_API_TOKEN: process.env.CODEX_SWARM_API_TOKEN
      ? "<redacted>"
      : null,
  },
};
