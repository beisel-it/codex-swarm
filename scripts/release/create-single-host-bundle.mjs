...
const stageParent = await mkdtemp(path.join(os.tmpdir(), "codex-swarm-release-"));
...
await run("npm", ["install", "--omit=dev", "--ignore-scripts", "--no-fund", "--no-audit"], stageRoot);
await runShell(
...
await wireInternalPackage(stageRoot, "contracts", path.join(repoRoot, "packages", "contracts", "dist", "src"));
await wireInternalPackage(stageRoot, "orchestration", path.join(repoRoot, "packages", "orchestration", "dist", "src"));
await wireInternalPackage(stageRoot, "worker", path.join(repoRoot, "apps", "worker", "dist", "src"));
await wireInternalPackage(stageRoot, "database", path.join(repoRoot, "packages", "database", "dist", "src"));
...
await run("tar", ["-czf", outputTarball, "-C", stageParent, bundleBaseName], repoRoot);
console.log(outputTarball);

async function wireInternalPackage(stageRoot, packageName, sourceDir) {
  const scopedDir = path.join(stageRoot, "node_modules", "@codex-swarm", packageName);
